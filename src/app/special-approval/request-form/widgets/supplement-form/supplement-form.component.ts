...
@Component({
  selector: 'special-supplement-form',
  templateUrl: './supplement-form.component.html',
  styleUrls: ['./supplement-form.component.scss']
})
export class SupplementFormComponent implements OnInit, OnChanges {
  ...
  @Output() onApSubmit = new EventEmitter<string>();
  ...
  get notify() {
    return this.formValues.get('notify');
  }

  get notifier() {
    return this.formValues.get('notifier');
  }

  get chatUsers() {
    return this.formValues.get('chatUsers');
  }

  get remark() {
    return this.formValues.get('remark');
  }

  get attachments() {
    return this.formValues.get('attachments') ;
  }
  ...
  //提交至 request-form.component 父组件，触发审批事件
  onSubmit(action) {
