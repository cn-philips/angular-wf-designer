...
export class OitRealtimeComponent implements OnInit, AfterViewInit {
  ...
  getTeamList() {
    ...
    if (modalitySet.size === 0 || modalitySet.size === 3) {
      teams = this.dashboardService.teamList
    } else {
      teams = this.dashboardService.teamList.filter(({ modality }) => modalitySet.has(modality))
    }
    if (teams.length > 0) {
  ...
  getChartData() {
    ...
    const { category, series, team, submitDate, modalityList } = this.formValues.value
    ...
    const params = {
      group,
      submitStartDate: startDate ? moment(startDate).format('YYYY-MM-DD') : '',
      submitEndDate: endDate ? moment(endDate).format('YYYY-MM-DD') : '',
      team: this.tabIndex === 1 ? team : '',
      modalityList: modalityList.filter(({ checked }) => checked).map(({ value }) => value),
    }

    this.subscribes = this.dashboardService.getOitRealTime(params).subscribe(({ data }) => {
      this.chartData = data
      if (this.tabIndex === 0) {
        this.handleOverviewData()
      } else {
  ...
  handleOverviewData() {
    ...
    const yAxisData = this.chartData.reduce((calc, 
      { 
        ...
        jdzbbAmount, qtCount, qtAmount, companyCount, contractAmount
      }
    ) => {
  ...
  handleSummaryData() {
    ...
    if (isStatusSeries) {
      ...
      this.chartData.forEach(({ smallArea, bmc, bmcCount, bmcAmount, companyCount, contractAmount }) => {
        this.totalAmount += contractAmount
        this.totalCount += companyCount
