...
import { HttpService, ReportExportService } from '@core/services';
import { of } from 'rxjs';
...
export class ReportComponent implements OnInit {
  ...
  toggleCollapse(): void {
    ...
    this.collapseText = this.isCollapse ? 'Collapse 收起' : 'Expand 展开';
  ...
  refreshTable(flag?: string) {
    ...
    this.http
      .post('/act/report/pageQuery', this.queryParams)
      .subscribe((res) => {
        if (res.code === '0000') {
  ...
  exportReport(dataArray: any[], onlyKeyCols?: boolean): void {
    ...
    let orderedHeader = Object.keys(this.keyRemap).map(
      (item) => this.keyRemap[item]
    );
    let param = {
  ...
  replaceKeyInObjectArray(a, replaceMap) {
    return a.map((o) =>
      Object.keys(o)
        .map((key) => ({ [replaceMap[key] || key]: o[key] }))
  ...
  initSelectList() {
    ...
    //临床细分list
    this.http
      .post('/act/masterdata/queryJsonByCondition/productclass', {
        status: '1',
      })
      .subscribe((res) => {
        if ('0000' == res.code) {
