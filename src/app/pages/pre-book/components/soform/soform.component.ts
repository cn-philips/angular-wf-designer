...
export class SoformComponent implements OnInit {
  ...
  constructor(public activatedRouter: ActivatedRoute, private router: Router, private fb: FormBuilder, private message: NzMessageService, private http: HttpService,private routerExtend: RouterExtendService) { }
  ...
  submit() {
    ...
    this.http.post(url, param).subscribe(res => {
      if (res.code == "0000") {
        ...
        this.routerExtend.back();
        // this.router.navigate(["/ecos/om-supplement"]);
      }
      else {
        this.message.create('error', res.msg);
      }
