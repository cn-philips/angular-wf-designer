...
import { decodeString, NumberThousandth, chNumber } from "assets/js/tools";
...
export class EmpComponent implements OnInit {
  ...
  ngOnInit() {
    this.activeRoute.queryParams.subscribe((params: Params) => {
      ...
      if (this.mainid != null && this.mainid != "") this.getData();
    });
  }
  //获取数据
  getData() {
    ...
    this.http.get(url + this.mainid).subscribe((res) => {
      ...
      if (
        this.data.tenderPriceCurrency != null &&
        this.data.tenderPriceCurrency != ""
      ) {
        this.data.tenderPriceCurrency = chNumber(this.data.tenderPriceCurrency);
        this.data.tenderPriceCurrency = NumberThousandth(
          this.data.tenderPriceCurrency
        );
      }
      if (
      ...
      if (
        this.data &&
        this.data.performanceBonds != "" &&
        this.data.performanceBonds != null
      ) {
        this.data.performanceBonds = chNumber(this.data.performanceBonds);
        this.data.performanceBonds = NumberThousandth(
          this.data.performanceBonds
        );
      }
      // this.data = res.data;
