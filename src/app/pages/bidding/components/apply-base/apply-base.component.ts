...
import { formatDatesNow } from "@core/util/tools";
import { NzMessageService } from "ng-zorro-antd";
import { ActivatedRoute } from "@angular/router";

@Component({
  selector: "app-applybase",
  templateUrl: "./apply-base.component.html",
  styleUrls: ["./apply-base.component.scss"],
})
export class ApplyBaseComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataBase.biddingManager = this.dataBase.biddingManager
      ? this.dataBase.biddingManager
      : localStorage.getItem("ng_philips_username");
  ...
  public DisableValidateForm() {
    if (
      this.dataBase &&
      (this.dataBase.baseDataFrom === "CP" ||
        this.dataBase.baseDataFrom === "CP Deal Form") &&
      this.dataBase.hospitalName !== "Stock" &&
      this.dataBase.hospitalName !== "stock"
    ) {
  ...
  public paymentMethod() {
    ...
    if (params.dictGroup != "") {
      this.http
        .post(`/act/ecom/dictData/queryGroupDictData`, params)
        .subscribe((rest) => {
          if (rest.code === "0000") {
            this.dataBase.paymentList = rest.data;
            let result = this.dataBase.paymentList.find(
              (val) => val.dictValue == this.dataBase.paymentDescription
            );
            if (!result) {
              const otherPaymentTerm = this.dataBase.paymentList.find(({ dictValue }) => dictValue.indexOf('其他') >= 0)
              if (otherPaymentTerm) {
                this.dataBase.paymentDescription = otherPaymentTerm.dictValue
                this.dataBase.paymentDescriptions = '其他'
              }
            }
          } else {
  ...
  cheakNumber(control: FormControl) {
    //数字的验证
    let res = /^[0-9]*$/;
      ...
      this.productData.map((e) => {
        if (this.arr && this.arr.tabList) {
          this.arr.tabList.map((arr) => {
            arr.radio = false;
            if (
              e.opportunityId == arr.opportunityId &&
              e.dealFormId == arr.dealFormId
            ) {
              arr.is = true;
            }
          });
  ...
  public async agentInit_Bidding() {
    if (
      ...
      this.dataBase.biddingNames !== undefined &&
      ...
      if (dealer && dealer.length > 0) {
        ...
        if (
          dealer[0].registeredAddress === "中国" ||
          dealer[0].registeredAddress === "中国香港"
        ) {
  ...
  public async agentInit_Agregent() {
    if (
      this.dataBase.agreementAgenName != null &&
      this.dataBase.agreementAgenName !== undefined &&
  ...
  public CkOpportunityIdAndDealFormId(opportunity) {
    for (let i = 0; i < this.productData.length; i++) {
      if (this.CpOrCrm === "CP Deal Form") {
        if (
          this.productData[i].opportunityId == opportunity.opportunityId &&
          this.productData[i].dealFormId == opportunity.dealFormId
        ) {
          return false;
        }
        // 判断是否重复simulationId
  ...
  public CkOpportunity(opportunity) {
    if (this.productData && this.productData.length > 0) {
      ...
      if (ho !== opportunity.hospitalId) {
        this.message.create(
          "error",
          opportunity.opportunityId + `客户名称不一致`
        );
        return false;
      }
    }
  ...
  public AddOpportunity(opportunity) {
    ...
    if (
      this.dataBase.baseDataFrom === "CRM" ||
      this.dataBase.baseDataFrom === "CP Simulation"
    ) {
      let url =
  ...
  async handleOk() {
    ...
    if (this.CpOrCrm !== "CP Deal Form") {
      ...
      this.http.post(urlCP, dataCP).subscribe(
        async (res) => {
  ...
  public getDataCP() {
    const urlCP = "/act/ecom/tender/application/tenderQueryOpportunityCP";
    if (this.oppSeach.accountName === "") this.oppSeach.accountName = null;
    if (this.oppSeach.opportunityId === "") this.oppSeach.opportunityId = null;
    if (this.oppSeach.opportunityName === "")
      this.oppSeach.opportunityName = null;
    if (this.oppSeach.dealFormId === "") this.oppSeach.dealFormId = null;
    if (this.oppSeach.simulationId === "") this.oppSeach.simulationId = null;
    const dataCP = {
      pageNo: this.paramsCP.pageNo,
      pageSize: this.paramsCP.pageSize,
    };
  ...
  public getDataCRM() {
    const urlCRM =
      "/act/ecom/tender/application/tenderQueryOpportunitySimulation";
    if (this.oppSeach.accountName === "") this.oppSeach.accountName = null;
    if (this.oppSeach.opportunityId === "") this.oppSeach.opportunityId = null;
    if (this.oppSeach.opportunityName === "")
      this.oppSeach.opportunityName = null;
    ...
    this.http.post(urlCRM, dataCRM).subscribe(
      (res) => {
  ...
  handleImportByDealFormId(dealFormId) {
    ...
    this.http
      .post("/act/ecom/tender/application/tenderQueryOpportunityCP", dataCP)
      .subscribe(
        ({ code, data, msg }) => {
          if (code === "0000") {
            if (data) {
              let result = data.rows.find((i) => i.dealFormId === dealFormId);
              if (result) {
                this.Ckdata = result;
                this.CpOrCrm = "CP Deal Form";
                this.handleOk();
    ...
    this.http
      ...
      .subscribe(
        ({ code, data, msg }) => {
          if (code === "0000") {
            if (data) {
              let result = data.rows.find(
                (i) => i.simulationId === simulationId
              );
              if (result) {
                this.Ckdata = result;
                this.CpOrCrm = "CP Simulation";
