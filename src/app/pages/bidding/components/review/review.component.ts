  ...
  submit() { //通过
    ...
    if (this.labarr.technicalTerms) {
        ...
        for (let i = 0; i < this.labarrs.bmclist.length; i++) {
          ...
          if (this.dataBase.BMCExpert[bmc]) {
            for (let j = 0; j < this.dataBase.BMCExpert[bmc].length; j++) {
              ...
              if (
                this.labarrs.bmclist[i].name ===
                this.dataBase.BMCExpert[bmc][j].name
              ) {
                this.labarrs.bmclist[i].email =
                  this.dataBase.BMCExpert[bmc][j].email;
              }
      ...
      for (let c = 0; c < this.labarrs.bmclist.length; c++) {
        if (
          (this.labarrs.bmclist[c].email === "" ||
            this.labarrs.bmclist[c].email == null) &&
          !this.take
        ) {
    ...
    if (
      !this.take &&
      this.labarr.approvalBidSecurity &&
      (this.labarrs.approvallist === null ||
        this.labarrs.approvallist === undefined ||
        this.labarrs.approvallist.length < 1)
    ) {
      this.message.create(
        "error",
        "投标保证金及履约保证金额审批人异常，请稍后重试"
      );
      return;
    }
    ...
    this.labarrs.nonStandardTerms =
      this.labarr.nonStandardTerms == true ? 1 : 0;
    this.labarrs.logisticsTermsApproval =
      this.labarr.logisticsTermsApproval == true ? 1 : 0;
    this.labarrs.approvalAfterSales =
      this.labarr.approvalAfterSales == true ? 1 : 0;
    this.labarrs.approvalBidSecurity =
      this.labarr.approvalBidSecurity == true ? 1 : 0;
    this.labarrs.paymentMethod = this.labarr.paymentMethod == true ? 1 : 0;
    this.labarrs.provisionsInvolved =
      this.labarr.provisionsInvolved == true ? 1 : 0;
    ...
    let y = this.labarrs;
    if (
      y.nonStandardTerms == 1 &&
      y.logisticsTermsApproval == 0 &&
      y.approvalAfterSales == 0 &&
      y.approvalBidSecurity == 0 &&
      y.paymentMethod == 0 &&
      y.technicalTerms == 0 &&
      y.provisionsInvolved == 0
    ) {
    ...
    this.http.post(url, this.labarrs).subscribe(
      (res) => {
        if (res.code == "0000") {
          this.message.create("success", res.msg);
