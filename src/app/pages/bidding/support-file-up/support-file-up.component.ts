...
export class SupportFileUpComponent implements OnInit {
  ...
  public IsOtherInput() {
    if (
      this.othercheck ||
      (this.odata.other != null && this.odata.other !== "")
    ) {
      return true;
    }
    return false;
  ...
  getData() {
    ...
    this.http.post(url, parmar).subscribe(
      (res) => {
        if (res.data) {
          this.infor = res.data;
          ...
          this.odata.specialApprovalFileName =
            this.infor.specialApprovalFileName;
          ...
          this.odata.supplementaryFileNamess =
            this.infor.supplementaryFileNamess;
          ...
          this.odata.biddingNotificationSignTime =
            this.infor.biddingNotificationSignTime; //后补中标通知书的签订时间
          this.odata.remarks = this.infor.remarks;
          this.odata.bidAnnouncementPrice =
            this.flag == 1
              ? NumberThousandth(this.infor.bidAnnouncementPrice)
              : this.infor.bidAnnouncementPrice;
          this.odata.bidAnnouncementCurrency =
            this.infor.bidAnnouncementCurrency;
          if (this.odata.other) {
          ...
          this.viewData2(
            "supplementaryFiless",
            "filesupplementssList",
            "supplementaryFileNamess"
          );
          this.viewData2(
            "bidAnnouncement",
            "bidAnnouncementList",
            ...
            for (let p1 = 0; p1 < res.data.biddingProductlist.length; p1++) {
              if (res.data.biddingProductlist[p1].biddingProductlist) {
                for (
                  let p2 = 0;
                  p2 <
                  res.data.biddingProductlist[p1].biddingProductlist.length;
                  p2++
                ) {
                  if (
                    res.data.biddingProductlist[p1].biddingProductlist[p2]
                      .biddingPrice
                  ) {
                    res.data.biddingProductlist[p1].biddingProductlist[
                      p2
                    ].biddingPrice = this.chNumber(
                      res.data.biddingProductlist[p1].biddingProductlist[p2]
                        .biddingPrice
                    );
                    res.data.biddingProductlist[p1].biddingProductlist[
                      p2
                    ].biddingPrice = NumberThousandth(
                      res.data.biddingProductlist[p1].biddingProductlist[p2]
                        .biddingPrice
                    );
                  }
  ...
  submitForm = (e) => {
    ...
    if (e == 1) {
      ...
      // 缺失中标通知书
      if (
        this.odata.bidWinningNotice == true ||
        this.odata.bidWinningNotice === "true"
      ) {
      ...
      // 缺失中标公告
      if (
        this.odata.bidWinningAnnouncement == true ||
        this.odata.bidWinningAnnouncement === "true"
      ) {
        if (
          par.bidWinningAnnouncement === "" ||
          par.bidWinningAnnouncement == null
        ) {
      ...
      if (this.othercheck == true || this.othercheck === "true") {
        if (
          (par.supplementaryFile === "" || par.supplementaryFile == null) &&
          (par.supplementaryFiles === "" || par.supplementaryFiles == null) &&
          (par.supplementaryFiless === "" || par.supplementaryFiless == null)
        ) {
    ...
    this.http.post("/act/biddingcheckSupportFileDone", par).subscribe(
      (e) => {
  ...
  inItData() {
    ...
    if (
      this.dataInfor.biddingProductlist &&
      this.dataInfor.biddingProductlist.length > 0
    ) {
      this.viewData(
        "bidWinningNotice",
        "fileNoticeList",
        "bidWinningNoticeName"
      );
      this.viewData(
        "bidWinningAnnouncement",
        "fileAnnouncementList",
        "bidWinningAnnouncementName"
      );
      this.viewData(
        "commitmentDocument",
        "filecommitmentList",
        "commitmentDocumentName"
      );
      ...
      this.dataInfor.biddingProductlist.map((res) => {
        res.productInformations &&
          (res.biddingProductlist = [...res.productInformations]);
        delete res.productInformations;
        res.biddingProductlist.map((vals) => {
          vals.marketBundleName = !vals.marketBundleName
            ? vals.productType
            : vals.marketBundleName;
          // this.validateForm.addControl(
          //   vals.id,
          //   new FormControl({value: 'Nancy', disabled:disa}, Validators.required)
          // );
          vals.biddingPrice = vals.biddingPrice
            ? vals.biddingPrice.toString()
            : "";
          vals.checked =
            vals.checked != undefined || vals.checked != null
              ? vals.checked
              : false;
          vals.productInformations &&
            (vals.biddingProductlist = [...vals.productInformations]);
  ...
  public IsspecialApprovalFile() {
    ...
    if (
      this.odata.bidWinningNotice ||
      this.odata.bidWinningAnnouncement ||
      this.odata.demandLetter ||
      this.odata.otherTypes ||
      this.odata.other
    ) {
      return true;
    }
  ...
  CkUrlLength() {
    if (this.mainid_winList && this.mainid_winList.length > 0) {
      for (let i = 0; i < this.mainid_winList.length; i++) {
        /* 判断子流程和主流程是否都为空 */
        if (
          this.mainid_winList[i].jdChildMainId ||
          this.mainid_winList[i].jdMainId
        ) {
          return true;
        }
      }
