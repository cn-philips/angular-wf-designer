...
export class BidComponent implements OnInit {
  ...
  getData() {
    ...
    this.http.get(url + this.mainid).subscribe((res) => {
      ...
      if (
        this.infor.tenderPriceCurrency != null &&
        this.infor.tenderPriceCurrency != ""
      ) {
        this.infor.tenderPriceCurrency = chNumber(
          this.infor.tenderPriceCurrency
        );
        this.infor.tenderPriceCurrency = NumberThousandth(
          this.infor.tenderPriceCurrency
        );
      }
      if (
        this.infor &&
        this.infor.totalPrice != "" &&
        this.infor.totalPrice != null
      ) {
      ...
      if (
        this.infor &&
        this.infor.performanceBonds != "" &&
        this.infor.performanceBonds != null
      ) {
        this.infor.performanceBonds = chNumber(this.infor.performanceBonds);
        this.infor.performanceBonds = NumberThousandth(
          this.infor.performanceBonds
        );
      }
...
//   * */
  isChan() {
    if (
      this.infor &&
      this.infor.tenderAuthorization == "private" &&
      this.infor.businessType == "DIRECT"
    ) {
      return false;
    }
    return true;
  ...
  save(e) {
    ...
    if (this.data.process === "approved" && e == 1) {
      ...
      if (price != null && !isNaN(price) && this.data.biddingProductlist) {
        for (let li = 0; li < this.data.biddingProductlist.length; li++) {
          for (
            let li2 = 0;
            li2 < this.data.biddingProductlist[li].biddingProductlist.length;
            li2++
          ) {
            // tslint:disable-next-line:radix
            let price_i = parseFloat(
              this.data.biddingProductlist[li].biddingProductlist[li2]
                .biddingPrice
            );
      ...
      if (this.data.biddingProductlist) {
        for (let li = 0; li < this.data.biddingProductlist.length; li++) {
          for (
            ...
            // 价格保留两位小数
            this.data.biddingProductlist[li].biddingProductlist[
              li2
            ...
            if (
              this.data.biddingProductlist[li].biddingProductlist[li2].checked
            ) {
              ckli = true;
              break;
            }
    ...
    if (
      ...
      (this.data.isUsdOrRmb == "" || this.data.isUsdOrRmb == null) &&
      e !== 0
    ) {
    ...
    if (
      this.isChan() &&
      this.data.process == "approved" &&
      (this.data.bidWinningTime == "" || this.data.bidWinningTime == null) &&
      e !== 0
    ) {
    ...
    if (
      this.data.process == "approved" &&
      (this.data.biddingPrice == "" || this.data.biddingPrice == null) &&
      e !== 0
    ) {
    ...
    if (
      this.data.process == "approved" &&
      (this.data.isSpecial == "" || this.data.isSpecial == null) &&
      e !== 0
    ) {
    ...
    if (this.data.bidWinningTime != null && this.data.bidWinningTime != "") {
      ...
      this.data.bidWinningTime = formatDatesNow(
        this.data.bidWinningTime
      ).toString();
    } else {
        ...
        if (res.data.biddingProductlist) {
          for (let p1 = 0; p1 < res.data.biddingProductlist.length; p1++) {
            if (res.data.biddingProductlist[p1].biddingProductlist) {
              for (
                let p2 = 0;
                p2 < res.data.biddingProductlist[p1].biddingProductlist.length;
                p2++
              ) {
                if (
