...
import { stringIndexof, delcommafy, floatAdd, floatMultiply, fomatFloat, floatSub,disreduce } from "@core/util/tools"
import {
  FormBuilder,
  FormGroup,
  FormControl,
...
@Component({
  ...
  styleUrls: ["./pre-order-examine.component.scss"],
})
...
export class PreorderexamineComponent implements OnInit {
  ...
  public editPreTable: any = false; //控制产品层级是否可以编辑
  ...
  public status; //流程装态
  public procInstId;
  public tabList = ['pending-tab', 'complete-tab', 'approval-record'];
  productModelInfo = {
  ...
  getData(data) {
    ...
    // if (localStorage.getItem('contractPriceAuthority') == 'true') {
    //   this.priceApprovalData.patchValue({
    //     ...data.preparationInfo
    //   })
    // } else {
    ...
    //   orderInfo.forEach(value => {
    //     value.financialSolutionCny = ''
  ...
  async preSubmit(parm) {
    ...
    const { applyId, processInstanceTaskId, productModelInfo, processStatus, modality, cycleGroup, bigArea, smallArea, accountFrom, baseInfoFrom, baseInfoTable, contractBuyerFrom, dealerFrom, endUserFrom, foreignFrom, orderInfo, priceApproval } = data;
    ...
    const orderInfos = orderInfo.map((val => {
      const { endUserinfo, productModelInfo, foreignInfo, accountFrom, mainTrems, otherTerms, orderBaseinfo, speciallyTerms, orderSalesinfo, marketBundleInfo } = val
  ...
  async addProduct(val) {
    ...
    let departmentListFirst = await this.serveice.departmentItemList()
    val.forEach((vals, index) => {
      ...
      vals.marketBundleInfo.forEach((a, i) => {
    ...
    this.orderInfo.controls.forEach((item, index) => {
      const orderSalesinfo = item.get('orderSalesinfo') as FormGroup;
      ...
      if (compareIgnoreSensitiveCase(orderSales ,this.user )&& this.flag == '0') {
        ...
        this.serveice.paymentMethod(params).then(paymentData => {
  ...
  createOrder(val: any, index) {
    ...
    const orderBaseinfo = {
      ...
      dealerRequestLetterRequired: [{ value: false, disabled: true }],
  ...
  createProdut(val: any, index, currencySystem) {
    ...
    const group = {
      ...
      firstLevelDepartment: [{ value: val.firstLevelDepartment, disabled: false }],//一级科室
      secondaryDepartment: [{ value: val.secondaryDepartment, disabled: false }],//二级科室
      marketBundleAmount: [{ value: val.marketBundleAmount, disabled: false }],//数量
      productModel: [{ value: val.productModel, disabled: false }],//产品型号
      medicalDeviceName: [{ value: val.medicalDeviceName, disabled: false }],//医疗器械名称
      nmpaNum: [{ value: val.nmpaNum, disabled: false }],//nmpaNum证号
      nmpaValidityDate: [{ value: val.nmpaValidityDate, disabled: false },],//NMPA证书有效期
      ...
      businessOpportunityHierarchyLink: [{ value: val.businessOpportunityHierarchyLink, disabled: true }, [Validators.required]],//商机层级链接
      businessOpportunityHierarchyTitle: [{ value: "", disabled: true }], //商机链接提示
      psm: [{ value: val.psm, disabled: false },], //psm
      netPrice: [{ value: val.netPrice, disabled: false },],//Net Price审批价
  ...
  computContral() {
    this.orderInfo.controls.forEach((element, index) => {
      ...
      if (!['其他（请在备注处描述实际付款方式）','其他（将触发非标付款方式审批流）'].includes(paymentProvision)  && paymentProvision != '远期信用证（请在备注处注明信用证期限及开证行）') {
        orderBaseinfo.patchValue({
          creditCny: 0,
          creditCnyNet: 0,
          creditUsd: 0,
