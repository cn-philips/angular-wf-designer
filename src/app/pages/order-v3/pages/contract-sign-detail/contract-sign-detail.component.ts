...
@Component({
  selector: "app-contract-sign-detail",
  templateUrl: "./contract-sign-detail.component.html",
  styleUrls: ["./contract-sign-detail.component.scss"],
})
export class ContractSignDetailComponent implements OnInit {
  subTierSubject = new Subject();
  info: any = {};
  contractSignFileList: any = [];
  contractId: any = "";
  ...
  baseInfoFrom = {
    ...
    approvalAreaConfiguration: [
      { value: null, disabled: !this.editBase },
      [Validators.required],
    ], //审批区域配置
  ...
  foreignFrom = {
    ...
    foreignTradeCorpSameRelatedDealer: [
      { value: null, disabled: !this.editBase },
    ], //外贸公司与经销商关联公司相同
  ...
  baseInfoTable = {
    ...
    installationWarrantySecondaryApproval: [
      { value: null, disabled: !this.editBase },
    ], //安装下一级审核 //无
  ...
  signFileForm = {
    ...
    contractConfirmedDate: [
      { value: null, disabled: this.editable },
      [Validators.required],
    ], // 合同确认日期
  ...
  init() {
    ...
    this.processInstanceTaskId =
      this.activatedRouter.queryParams["value"].processInstanceTaskId;
    this.flag = this.activatedRouter.queryParams["value"].flag;
    this.procInstId = this.activatedRouter.queryParams["value"].procInstId;
    this.sign = this.activatedRouter.queryParams["value"].sign;
    this.signatureStatus =
      this.activatedRouter.queryParams["value"].signatureStatus;
    this.zslSignSupplement =
      this.activatedRouter.queryParams["value"].zslSignSupplement;
    this.contractId = this.activatedRouter.queryParams["_value"].id;
    ...
    if (this.applyId) {
      ...
      this.serveice.queryContact(this.applyId).then((res) => {
        ...
        this.info = data;
        this.contractSignFileList = contractSignInfo.zslNotSignedFile
          ? contractSignInfo.zslNotSignedFile
          : [];
        this.getData(data);
        ...
        if (this.flag === "1") {
          ...
          if (this.sign === "1" && applySignatureBtn) {
            this.applySignatureBtn = true;
            this.signFileFormData.get("zslNotSignedFile").enable();
            this.setFieldRequired("zslNotSignedFile");
          }
  ...
  getData(param) {
    ...
    this.formValue.patchValue({
      ...
      processInstanceTaskId: param.processInstanceTaskId
        ? param.processInstanceTaskId
        : this.processInstanceTaskId,
    ...
    this.oaAddInfoData.patchValue({
      ...orderSummaryInfo,
      customerRequestLetterDate: orderSummaryInfo.customerRequestLetterDate
        ? orderSummaryInfo.customerRequestLetterDate.slice(0, 10)
        : null,
    });
    ...
    const signedFileRelationList = this.signFileFormData.get(
      "signedFileRelationList"
    ) as FormArray;
    if (Array.isArray(contractSignInfo.signedFileRelationList)) {
      contractSignInfo.signedFileRelationList.forEach(
        ({ signedFileId, unsignedFileId }) => {
          signedFileRelationList.push(
            this.fb.group({ signedFileId, unsignedFileId })
          );
        }
      );
    }
    ...
    this.productModelInfoData.patchValue({
      ...
      this.baseInfoFromData.patchValue({
        solutionSalesNameModel: contractInfo.solutionSalesEmail
          ? `${contractInfo.solutionSalesName}(${contractInfo.solutionSalesEmail})`
          : "",
        actualSalesNameModel: contractInfo.actualSalesEmail
          ? `${contractInfo.actualSalesName}(${contractInfo.actualSalesEmail})`
          : "",
    ...
    if (this.marketBundleInfo.length === 0) {
      marketBundleInfo.map((val, index) => {
        this.marketBundleInfo.push(
          this.createProdut(val, index, currencySystem)
        );
      });
    ...
    if (this.flag == "0") {
      if (this.status == "ecos_oit_order_sign") {
        const cpDealOrderId =
          this.priceApprovalData.getRawValue().cpDealOrderId;
        if (!cpDealOrderId) {
          this.serveice
            .getCpDealOrderId(contractInfo.oitOrderId)
            .subscribe((item) => {
              if (item.code == "0000") {
                this.priceApprovalData.patchValue({
      ...
      setTimeout(() => {
        ...
        const contractSignForm = this.contractSignFormData.getRawValue();
        this.contractSignFormData.patchValue({
          accountName: contractSignInfoNow.accountName
            ? contractSignInfoNow.accountName
            : accountFrom.accountName,
          bankName: contractSignInfoNow.bankName
            ? contractSignInfoNow.bankName
            : accountFrom.bankName,
          accountNo: contractSignInfoNow.accountNo
            ? contractSignInfoNow.accountNo
            : accountFrom.accountNo,
          registrationAddress: contractSignInfoNow.registrationAddress
            ? contractSignInfoNow.registrationAddress
            : accountFrom.registrationAddress,
          accountPhoneFax: contractSignInfoNow.accountPhoneFax
            ? contractSignInfoNow.accountPhoneFax
            : accountFrom.accountPhoneFax,
          taxNum: contractSignInfoNow.taxNum
            ? contractSignInfoNow.taxNum
            : accountFrom.taxNum,
          purchaseOrderSignatory: contractSignForm.purchaseOrderSignatory
            ? contractSignForm.purchaseOrderSignatory
            : dealerform.purchaseOrderSignatory,
          purchaseOrderSignatoryPosition:
            contractSignForm.purchaseOrderSignatoryPosition
              ? contractSignForm.purchaseOrderSignatoryPosition
              : dealerform.purchaseOrderSignatoryPosition,
          importAgreementSignName: contractSignForm.importAgreementSignName
            ? contractSignForm.importAgreementSignName
            : foreignFrom.importAgreementSignName,
          importAgreementSignPosition:
            contractSignForm.importAgreementSignPosition
              ? contractSignForm.importAgreementSignPosition
              : foreignFrom.importAgreementSignPosition,
          contractBuyerSignatory: contractSignForm.contractBuyerSignatory
            ? contractSignForm.contractBuyerSignatory
            : contractBuyerFrom.contractBuyerSignatory,
          contractBuyerSignatoryPosition:
            contractSignForm.contractBuyerSignatoryPosition
              ? contractSignForm.contractBuyerSignatoryPosition
              : contractBuyerFrom.contractBuyerSignatoryPosition,
          addressee: contractSignForm.addressee
            ? contractSignForm.addressee
            : accountFrom.recipient,
          addresseeTel: contractSignForm.addresseeTel
            ? contractSignForm.addresseeTel
            : accountFrom.recipientPhone,
          invoiceMailingInformation: contractSignForm.invoiceMailingInformation
            ? contractSignForm.invoiceMailingInformation
            : accountFrom.invoicesDeliverAddress,
        });
        if (this.status == "ecos_oit_order_sign") {
    ...
    this.priceApprovalData.patchValue({
      tradeInTotal:
        contractInfo.currencySystem == "CNY"
          ? contractInfo.orderTradeInCnyNet != null &&
            contractInfo.orderTradeInCnyNet != ""
            ? contractInfo.orderTradeInCnyNet
            : 0
          : contractInfo.orderTradeInUsd != null &&
            contractInfo.orderTradeInUsd != ""
          ? contractInfo.orderTradeInUsd
          : 0,
      rebateTotal:
        contractInfo.currencySystem == "CNY"
          ? contractInfo.orderRebateCnyNet != null &&
            contractInfo.orderRebateCnyNet != ""
            ? contractInfo.ordeRerbateCnyNet
            : 0
          : contractInfo.orderRebateUsd != null &&
            contractInfo.orderRebateUsd != ""
          ? contractInfo.orderRebateUsd
          : 0,
      financialSolutionName: contractInfo.financialSolutionOther
        ? contractInfo.financialSolutionOther == "null"
          ? ""
          : contractInfo.financialSolutionOther
        : "",
      tradeInTotalOS:
        contractInfo.currencySystem == "CNY"
          ? contractInfo.orderChapterTradeInNetCny != null &&
            contractInfo.orderChapterTradeInNetCny != ""
            ? contractInfo.orderChapterTradeInNetCny
            : 0
          : contractInfo.orderTradeInUsd != null &&
            contractInfo.orderChapterTradeInUsd != ""
          ? contractInfo.orderChapterTradeInUsd
          : 0,
      rebateTotalOS:
        contractInfo.currencySystem == "CNY"
          ? contractInfo.orderChapterRebateNetCny != null &&
            contractInfo.orderChapterRebateNetCny != ""
            ? contractInfo.orderChapterRebateNetCny
            : 0
          : contractInfo.orderChapterRebateUsd != null &&
            contractInfo.orderChapterRebateUsd != ""
          ? contractInfo.orderChapterRebateUsd
          : 0,
    });
    this.priceData = {
    ...
    if (this.baseInfoFromData.getRawValue().businessModel == "DISTRIBUTOR") {
      ...
      setTimeout(() => {
        const subTierDisbaled = !(
          this.flag == "0" &&
          ["ecos_oit_order_os_input", "ecos_oit_order_sign"].includes(
            this.status
          )
        );
  ...
  getdistributorDate() {
    ...
    this.serveice
      .findDealersByPageValid({ dealerName: dealerName })
      .then((item) => {
        if (item.code == "0000") {
          ...
          if (rows.length > 0) {
            ...
            if (ddpStatus != "通过") {
              setTimeout(() => {
                this.dealerFromData.patchValue({
                  dealerDdpStatus: "不通过",
                  dealerDdpValidityDate: moment(
                    rows[0].mdtdealerddpexpiredate
                  ).format("YYYY-MM-DD"),
                });
              }, 1000);
            } else {
  ...
  async getIepoolDate() {
    ...
    if (companyNotInIePool) {
      ...
      const dateAndValid = await this.serveice.findEcosiepool({
        corporateName: foreignTradeCorpName,
      });
  ...
  async orderSubmit(parm, isApprovalReject: boolean = false) {
    ...
    signFileForm.contractConfirmedDate =
      ...
      signFileForm.contractConfirmedDate != ""
        ...
        : null;
    dealerFrom.dealerDdpValidityDate =
      dealerFrom.dealerDdpValidityDate != null &&
      dealerFrom.dealerDdpValidityDate != ""
        ? moment(dealerFrom.dealerDdpValidityDate).format("YYYY-MM-DD hh:mm:ss")
        : null;
    foreignFrom.foreignTradeCorpDdpValidityDate =
      foreignFrom.foreignTradeCorpDdpValidityDate != null &&
      foreignFrom.foreignTradeCorpDdpValidityDate != ""
        ? moment(foreignFrom.foreignTradeCorpDdpValidityDate).format(
            "YYYY-MM-DD hh:mm:ss"
          )
        : null;
    oaAddInfo.customerRequestLetterDate =
      oaAddInfo.customerRequestLetterDate != null &&
      oaAddInfo.customerRequestLetterDate != ""
        ? moment(oaAddInfo.customerRequestLetterDate).format(
            "YYYY-MM-DD hh:mm:ss"
          )
        : null;
    baseInfoFrom.requiredArrivalDate =
      baseInfoFrom.requiredArrivalDate != null &&
      baseInfoFrom.requiredArrivalDate != ""
        ? moment(baseInfoFrom.requiredArrivalDate).format("YYYY-MM-DD hh:mm:ss")
        : null;
    baseInfoFrom.estimateInstallationDate =
      baseInfoFrom.estimateInstallationDate != null &&
      baseInfoFrom.estimateInstallationDate != ""
        ? moment(baseInfoFrom.estimateInstallationDate).format(
            "YYYY-MM-DD hh:mm:ss"
          )
        : null;
    this.accountFromData.patchValue({
    ...
    if (remarkFrom.attachmentIds && remarkFrom.attachmentIds.length > 0) {
      remarkFrom.attachmentIds = remarkFrom.attachmentIds.map(
        (val) => val.fileId
      );
    }
    ...
    if (parm == "approved") {
      if (
        param.processStatus === "ecos_oit_order_sign" &&
        this.baseInfoFromData.getRawValue().businessModel == "DISTRIBUTOR"
      ) {
        const subTierInfo = this.formValue
          .get("dealerFrom")
          .get("subTierInfo") as FormArray;
        if (subTierInfo.invalid) {
          this.modalService
            .error({
              nzTitle: "提示",
              nzContent:
                "经销商黑名单校验不通过，请上传必要的支持文件和备注后，再作提交",
            })
      ...
      if (baseInfoFrom.businessModel == "DISTRIBUTOR") {
        ...
        if (!this.allowPass) {
          const dateAndValid = await this.serveice.findDealersByPageValid({
            dealerName: dealerFrom.dealerName,
          });
          if (dateAndValid.code == "0000") {
            const rows = dateAndValid.data.rows;
            if (rows.length > 0) {
              const ddpValidUntil = standardTime(
                rows[0].mdtdealerddpexpiredate
              );
              const ddpStatus = isadopt(ddpValidUntil);
              if (ddpStatus != "通过") {
      ...
      if (priceApproval.currencySystem == "USD") {
        if (!this.allowPass) {
          if (foreignFrom.companyNotInIePool) {
            ...
            const dateAndValid = await this.serveice.findEcosiepool({
              corporateName: foreignFrom.foreignTradeCorpName,
            });
  ...
  setSignRequired() {
    ...
    if (baseInfo.businessModel === "DIRECT") {
      this.contractSignFormData
        .get("salesAgreementNo")
        .setValidators([Validators.required]);
  ...
  createProdut(val: any, index, currencySystem) {
    ...
    const group = {
      ...
      primaryOpportunity: [
        { value: val.primaryOpportunity, disabled: !this.editBase },
      ], //是否主机
      ...
      clinicalClassification: [
        { value: val.clinicalClassification, disabled: false },
      ], //临床分类
      firstLevelDepartment: [
        { value: val.firstLevelDepartment, disabled: !this.editBase },
      ], //一级科室
      secondaryDepartment: [
        { value: val.secondaryDepartment, disabled: !this.editBase },
      ], //二级科室
      ...
      dtcDealerAgreementNo: [
        { value: val.dtcDealerAgreementNo, disabled: !this.editBase },
      ], //Dtc经销商协议号
      newDealerAgreementNo: [
        { value: val.newDealerAgreementNo, disabled: !this.editBase },
      ], //最新经销商协议号newDealerAgreementNo
      ...
      businessOpportunityHierarchyLink: [
        { value: val.businessOpportunityHierarchyLink, disabled: true },
      ], //商机层级链接
