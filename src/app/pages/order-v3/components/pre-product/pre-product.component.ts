...
import { FormGroup, Validator, FormBuilder, FormArray, Validators, FormControl, ValidationErrors } from "@angular/forms";
import { OrderV3Service } from "../../order-v3.service";
import { Router, ActivatedRoute } from '@angular/router';
import { stringIndexof, haveRolesArr, isadopt, standardTime,floatMultiply,fomatFloat,disreduce, removeRepeat, floatSub, returnFloat, floatDivide, floatAdd } from "@core/util/tools";
...
export class PreProductComponent implements OnInit {
  constructor(private service: OrderV3Service,
    ...
    this.service.productReceive.subscribe((vals) => {
      ...
      if (this.orderInfo && this.orderInfo.controls && this.orderInfo.controls.length > 0) {
        this.orderInfo.controls.forEach((val, index) => {
          ...
          const productModelInfo = this.orderInfo.at(index).get("productModelInfo") as FormGroup;
          const accountFrom = this.orderInfo.at(index).get("accountFrom") as FormGroup;
          const foreignInfo = this.orderInfo.at(index).get("foreignInfo") as FormGroup;
          const endUserinfo = this.orderInfo.at(index).get("endUserinfo") as FormGroup;
          mainTrems.disable();
          otherTerms.disable();
          ...
          const { orderSalesModality, orderOa, orderOaAgent, actualSalesEmail, actualSalesName, orderSalesProvince } = orderSalesinfo.getRawValue();
          const { biddingType, approvalAreaConfiguration, centralizedPurchasing, dealFormSalesCity, dealFormSalesPerformanceProvince, dealFormSalesModality, dealFormSalesBigArea, dealFormSalesSmallArea, dealFormSalesProvince, businessModel, dealFormSalesTeam } = this.baseInfoFrom.getRawValue() //招标类型
          ...
          const { isload, modality, cycleGroup, bigArea, smallArea } = this.formValue.getRawValue();
          ...
          if (orderSalesProvince != null && orderSalesProvince != undefined && orderSalesProvince != "") {
            this.selectCity(orderSalesProvince, index, false)
          }
          if ((this.status == undefined || this.status == "" || this.status == "ecos_oit_deal_submit" || this.status == "ecos_oit_deal_resubmit" || this.status == "ecos_status_draft") && (this.flag == '0' || this.flag == undefined)) {

            if (centralizedPurchasing == '0' && orderModality == dealFormSalesModality && isload) {
            ...
            else if (centralizedPurchasing == '0' && orderModality == dealFormSalesModality) {
              orderSalesinfo.patchValue({
                orderDisbled: true,
              })
            }
          ...
          */
          ...
          if ((orderModality == 'US' || orderModality == 'CC') && supportFileMissing == '0') {

            if (sampleCheck == '1') {
              speciallyTerms.get("endUserContract").setValidators(Validators.required);
            }
            else {
            ...
            if (hospitalType == '公立医院' && sampleCheck == '1') {
              speciallyTerms.get("bidWinningFile").setValidators(Validators.required);
            }
            else {
            ...
            if (businessModel == 'DISTRIBUTOR' && sampleCheck == '1') {
              speciallyTerms.get("projectAnalysisTable").setValidators(Validators.required);
            }
            else {
          ...
          if (orderSales != "" && orderSales != null && orderSales != undefined) {
          ...
          if (this.status == 'ecos_oit_deal_oa' && this.flag == '0') {
            ...
            if (orderOa == this.user || orderOaAgent == this.user) {
              ...
              const financialSolutionDisabled = orderBaseinfo.getRawValue().financialSolutionDisabled
              ...
              if (!financialSolutionDisabled) {
                ...
                {
                  orderBaseinfo.get("financialSolutionCnyNet").setValidators([Validators.required]);
                  orderBaseinfo.get("financialSolutionUsd").clearValidators();
                }
                else
                {
                  orderBaseinfo.get("financialSolutionUsd").setValidators([Validators.required]);
                  orderBaseinfo.get("financialSolutionCnyNet").clearValidators();
                }                
              }
              else {
              ...
              orderBaseinfo.get('sofonFile').setValidators([Validators.required]);
              orderBaseinfo.get('sofonNum').setValidators([Validators.required]);
              orderBaseinfo.patchValue({
              ...
              const { performanceBond, afterSalePrice, qualityGuarantee } = mainTrems.getRawValue();
              ...
              if (orderModality == "US") {
                ...
                if (businessModel == 'DISTRIBUTOR') {
                  orderBaseinfo.get("dealerSapCode").setValidators(Validators.required);
                  orderBaseinfo.get("dealerSapCode").markAsDirty();
                }
                orderSalesinfo.get("orderSalesSapCode").setValidators(Validators.required);
              ...
              if (performanceBond == '1') {
                mainTrems.get('performanceBondRemarks').setValidators(Validators.required);
                mainTrems.get('performanceBondRemarks').updateValueAndValidity();
              }
              if (afterSalePrice == '1') {
                mainTrems.get('afterSalePriceRemarks').setValidators(Validators.required);
                mainTrems.get('afterSalePriceRemarks').updateValueAndValidity();
              }
              if (qualityGuarantee == '1') {
                mainTrems.get('qualityGuaranteeRemarks').setValidators(Validators.required);
                mainTrems.get('qualityGuaranteeRemarks').updateValueAndValidity();
              }
              ...
              const { shipmentDelivery,
                ...
                otherOcap,
                other } = otherTerms.getRawValue()

              if (shipmentDelivery == '1') {
                otherTerms.get('shipmentDeliveryRemarks').setValidators(Validators.required);
                otherTerms.get('shipmentDeliveryRemarks').updateValueAndValidity();
              }
              if (installationWarranty == '1') {
                otherTerms.get('installationWarrantyRemarks').setValidators(Validators.required);
                otherTerms.get('installationWarrantyRemarks').updateValueAndValidity();
              }
              if (sitePreparation == '1') {
                otherTerms.get('sitePreparationRemarks').setValidators(Validators.required);
                otherTerms.get('sitePreparationRemarks').updateValueAndValidity();
              }
              ...
              const { supportFileMissing, amountDifference } = speciallyTerms.getRawValue()
              if (supportFileMissing == '1') {
                speciallyTerms.get('supportFileMissingRemarks').setValidators(Validators.required);
                speciallyTerms.get('supportFileMissingRemarks').updateValueAndValidity();
              }
              if (amountDifference == '1') {
                speciallyTerms.get('amountDifferenceRemarks').setValidators(Validators.required);
                speciallyTerms.get('amountDifferenceRemarks').updateValueAndValidity();
              }
  ...
  performanceBondModelChanges(event, i) {
    this.performanceBondChange('performanceBond', i, 'mainTrems', 'performanceBondRemarks');
  ...
  foreignInfoDDpstatus(i) { //验证ddp是否过期
    const foreignInfo = this.orderInfo.at(i).get("foreignInfo") as FormGroup;
    const { orderSameForeignTradeCorp, foreignTradeCorpDdpValidityDate } = foreignInfo.getRawValue();
    if (orderSameForeignTradeCorp == '0') {
      foreignInfo.get('foreignTradeCorpDdpStatus').enable();
      foreignInfo.get('foreignTradeCorpDdpStatus').setValidators([this.dDpstatusvalid, Validators.required])
  ...
  qualityGuaranteeModelChanges(event, i) {
    ...
    this.performanceBondChange('qualityGuarantee', i, 'mainTrems', 'qualityGuaranteeRemarks');
    const mainTrems = this.orderInfo.at(i).get('mainTrems') as FormGroup;
    if (event == 1) {
  ...
  afterSalePriceModelChanges(event, i) {
    this.performanceBondChange('afterSalePrice', i, 'mainTrems', 'afterSalePriceRemarks');
    const mainTrems = this.orderInfo.at(i).get('mainTrems') as FormGroup;
    if (event == 1) {
  ...
  shipmentDeliveryModelChanges(event, i) {
    this.performanceBondChange('shipmentDelivery', i, 'otherTerms', 'shipmentDeliveryRemarks');
  ...
  sitePreparationModelChanges(event, i) {
    this.performanceBondChange('sitePreparation', i, 'otherTerms', 'sitePreparationRemarks');
    const otherTerms = this.orderInfo.at(i).get('otherTerms') as FormGroup;
    if (event == 1) {
  ...
  installationWarrantyModelChanges(event, i) {
    this.performanceBondChange('installationWarranty', i, 'otherTerms', 'installationWarrantyRemarks');
    const otherTerms = this.orderInfo.at(i).get('otherTerms') as FormGroup;
    if (event == 1) {
  ...
  amountDifferenceModelChanges(event, i) {
    this.performanceBondChange('amountDifference', i, 'speciallyTerms', 'amountDifferenceRemarks');
    const speciallyTerms = this.orderInfo.at(i).get('speciallyTerms') as FormGroup;
    if (event == 1) {
  ...
  selectCity(param, i, flag: any = true) {
    const orderSalesinfo = this.orderInfo.at(i).get('orderSalesinfo') as FormGroup;
  ...
  onShowSelectActualModal(i) {
    const orderBaseinfo = this.orderInfo.at(i).get("orderBaseinfo") as FormGroup;
    const value = orderBaseinfo.getRawValue().orderModality;
    this.selectActualUser.show({ modality: value, role: 'Sales Rep/Mgr' }, true);
    this.index = i;
  }
  onClearOrder(i) {
    const orderBaseinfo = this.orderInfo.at(i).get("orderBaseinfo") as FormGroup;
    ...
    const otherTerms = this.orderInfo.at(i).get("otherTerms") as FormGroup;
    const speciallyTerms = this.orderInfo.at(i).get("speciallyTerms") as FormGroup;
    const { orderModality, cpDealOrderId, marketBundleId, totalContractPrice, sofonNum, sofonFile, currencySystem } = orderBaseinfo.getRawValue();
  ...
  onClearActualOrder(i) {
    const orderBaseinfo = this.orderInfo.at(i).get("orderSalesinfo") as FormGroup;
    orderBaseinfo.patchValue({
  ...
  onSelectuser(val) {
    const orderSalesinfo = this.orderInfo.at(this.index).get("orderSalesinfo") as FormGroup;
    ...
    if (approvalAreaConfiguration) {
      ...
      if (dealFormSalesModality == orderModality && this.user == val.email && (this.flag != '1')) {
   ...
   */
  isDisable(param, index, configOff: any = false) { //验证当前登录人与sale deal是否相同，禁用与否    
    // const filter = /[\u4E00-\u9FA5\uF900-\uFA2D]{1,}/;
    ...
    if (this.user == param && (this.status == 'ecos_oit_deal_sales' || this.status == "ecos_oit_deal_resubmit" || this.status == '' || this.status == undefined || this.status == 'ecos_status_draft') && this.flag != '1') {
      const { isload, modality, cycleGroup, bigArea, smallArea } = this.formValue.getRawValue()
      const { approvalAreaConfiguration, centralizedPurchasing, dealFormSalesCity, dealFormSalesPerformanceProvince, dealFormSalesModality, dealFormSalesBigArea, dealFormSalesSmallArea, dealFormSalesProvince, dealFormSalesTeam } = this.baseInfoFrom.getRawValue() //招标类型
    ...
    if (this.user == param && this.status == 'ecos_oit_deal_sales' && this.flag != '1') {//销售填写节点
  ...
  onSelectActualuser(val) {
    const orderBaseinfo = this.orderInfo.at(this.index).get("orderSalesinfo") as FormGroup;
    orderBaseinfo.patchValue({
  ...
  onShowSelectPrebookModal(i) {
    //弹出prebook弹窗口
    const orderBaseinfo = this.orderInfo.at(i).get("orderBaseinfo") as FormGroup;
    const marketBundleInfo = this.orderInfo.at(i).get("marketBundleInfo") as FormArray;
    ...
    this.selectPreBook.show({ opportunityId: opportunityId, marketBundleName: marketBundleName, marketBundleAmount: marketBundleAmount }, true);
    this.index = i;
  }
  onPrebookselect(val) {
    const orderBaseinfo = this.orderInfo.at(this.index).get("orderBaseinfo") as FormGroup;
  ...
  checkFormData() {
    ...
    for (const i in this.orderInfo.controls) {
      ...
      const orderSalesinfo = this.orderInfo.controls[i].get("orderSalesinfo") as FormGroup;
      const speciallyTerms = this.orderInfo.controls[i].get("speciallyTerms") as FormGroup;
      const foreignInfo = this.orderInfo.controls[i].get("foreignInfo") as FormGroup;
      const endUserinfo = this.orderInfo.controls[i].get("endUserinfo") as FormGroup;
      const orderBaseinfo = this.orderInfo.controls[i].get("orderBaseinfo") as FormGroup;
      const mainTrems = this.orderInfo.controls[i].get("mainTrems") as FormGroup;
      const otherTerms = this.orderInfo.controls[i].get("otherTerms") as FormGroup;
      const marketBundleInfo = this.orderInfo.controls[i].get("marketBundleInfo") as FormArray;
      const accountFrom = this.orderInfo.controls[i].get("accountFrom") as FormGroup;
  ...
  upDataAndValidity(param: FormGroup) { //更新状态不弹出提示
    for (const j in param.controls) {
      param.controls[j].updateValueAndValidity();
    }
  }
  ...
  selectConfig(systemRegion, i) {
    ...
    const region = this.saleRegions.find((region) => systemRegion === region.value)
  ...
  onForignselect(val) {
    const foreignInfo = this.orderInfo.at(this.index).get('foreignInfo') as FormGroup
    foreignInfo.reset();
    foreignInfo.patchValue({
      orderSameForeignTradeCorp: '0',
  ...
  onClearHospital(i) {
    const endUserinfo = this.orderInfo.at(this.index).get('endUserinfo') as FormGroup;
    endUserinfo.reset();
    endUserinfo.patchValue({
      orderSameEndUser: '0'
    })
  ...
  onHospitalselect(val) {
    ...
    endUserinfo.patchValue({
      ...
      usHta: val.category != '' && val.category != null && val.category.indexOf('US:HTA') != -1 ? '是' : '否'
    });
  ...
  isEnduser(index) {
    ...
    if ((orderSameEndUser == '0' && oitMode == "BIDDING" && orderModality == 'PD&IGT') || (orderSameEndUser == '0' && orderModality == 'US')) {
  ...
  isForeginSimilar(index) { //外贸公司与基础信息是否相同的必填字段验证

    const foreignInfo = this.orderInfo.at(index).get('foreignInfo') as FormGroup
    const orderSalesinfo = this.orderInfo.at(index).get('orderSalesinfo') as FormGroup;
    const { foreignTradeCorpSameDealer,
    ...
    if (orderSameForeignTradeCorp == '0' && this.flag != '1' && orderSales == this.user) {
      ...
      foreignInfo.get('foreignTradeCorpDdpStatus').enable();
      foreignInfo.get('foreignTradeCorpName').setValidators(Validators.required);
      foreignInfo.get('foreignTradeCorpDdpStatus').setValidators([this.dDpstatusvalid, Validators.required])
      ...
      foreignInfo.get('foreignTradeCorpTaxNum').setValidators(Validators.required);
      foreignInfo.get('foreignTradeCorpAddress').setValidators(Validators.required);
      foreignInfo.get('foreignTradeCorpContact').setValidators(Validators.required);
      foreignInfo.get('foreignTradeCorpEmail').setValidators(Validators.required);
      foreignInfo.get('importAgreementSignName').setValidators(Validators.required);
      foreignInfo.get('importAgreementSignPosition').setValidators(Validators.required);
      foreignInfo.get('foreignTradeCorpPhone').setValidators(Validators.required);
      foreignInfo.get('foreignTradeCorpDdpValidityDate').setValidators(Validators.required);
      foreignInfo.patchValue({
  ...
  showMagneticResonanceShieldingFileHost(i) {
    ...
    if (orderModality == 'PD&IGT' && marketBundleInfoData && marketBundleInfoData.length > 0) {
      const host = marketBundleInfoData.find(val => val.primaryOpportunity == 'true');
      if (host && host.marketBundleBmc == 'MR') {
        orderBaseinfo.get("magneticResonanceShieldingFile").setValidators(Validators.required);
        orderBaseinfo.patchValue({
      ...
      orderBaseinfo.get("magneticResonanceShieldingFile").updateValueAndValidity();
    }
  }
  showigtThirdPartyFileHost(i) {
    ...
    if (orderModality == 'PD&IGT' && marketBundleInfoData && marketBundleInfoData.length > 0) {
      const host = marketBundleInfoData.find(val => val.primaryOpportunity == 'true');
      if (host && host.marketBundleBmc == 'IGT-S') {
  ...
  igtChang(i) {
    ...
    if (orderModality == 'PD&IGT') {
      if (igtThirdPartySingle == '1') {
        orderBaseinfo.get('igtThirdPartyFile').setValidators(Validators.required);
      }
      else {
  ...
  actualSalesSetValidators(i) {
    ...
    const orderSalesinfo = this.orderInfo.at(i).get('orderSalesinfo') as FormGroup;
    const ordercentralizedPurchasing = orderBaseinfo.getRawValue().centralizedPurchasing
    if (ordercentralizedPurchasing == '1') {
  ...
  isSolutionSale(i) {
    ...
    const host = marketBundleInfos.find(val => val.primaryOpportunity == "true")
  ...
  ifrequiredFile(i) {
    ...
    const modality = orderBaseinfo.getRawValue().orderModality;
    if (modality == 'US') {
      orderBaseinfo.get('dealerRequestLetterFile').setValidators(Validators.required);
  ...
  ifrequiredDate(i) {
    ...
    const modality = orderBaseinfo.getRawValue().orderModality;
    if (modality == 'US') {
      orderBaseinfo.get('requiredArrivalDate').setValidators(Validators.required);
      orderBaseinfo.get('estimateInstallationDate').setValidators(Validators.required);
  ...
  */
  ...
  changPaymentProvision(event,i)
  {
    ...
    const orderBaseinfo = this.orderInfo.at(i).get('orderBaseinfo') as FormGroup;
    const { paymentProvision } = mainTrems.getRawValue();
    const {paymentCnyCp,paymentNetCnyCp,paymentUsdCp,creditCnyCp,creditCnyNetCp,creditUsdCp}=orderBaseinfo.getRawValue();
     if (paymentProvision == '远期信用证（请在备注处注明信用证期限及开证行）') {
        orderBaseinfo.patchValue({
          paymentCny: 0,
          paymentNetCny: 0,
  ...
  provisionChang(i) { //付款条款  
    ...
    const { paymentProvision } = mainTrems.getRawValue();
    if (paymentProvision == '其他（请在备注处描述实际付款方式）') {
      mainTrems.get('paymentProvisionRemarks').setValidators(Validators.required);
  ...
  isOtherTrain(i) {
    //其他条款是否必填*
    const otherTerms = this.orderInfo.at(i).get('otherTerms') as FormGroup;
    const { otherTrain, otherFine, otherIp, otherContractTemplate, otherOcap, other } = otherTerms.getRawValue();
    if (otherTrain == true || otherFine == true || otherIp == true || otherContractTemplate == true || otherOcap == true || other == true) {
      otherTerms.get("otherRemarks").setValidators(Validators.required);
  ...
  changForeignDate(val, i) {
    if(!this.isAutoChange) {
      ...
      if (
        ...
        (!select && this.flag != 1) ? foreignFrom.get("foreignTradeCorpDdpValidityDate").enable()
          : foreignFrom.get("foreignTradeCorpDdpValidityDate").disable();
      }
    }
  ...
  foreignformShow(i) {
    ...
    if (orderBaseinfo.getRawValue().currencySystem == 'USD') {
      ...
      if (foreignTradeCorpSameDealer == true && this.user == orderSales && (this.status == 'ecos_oit_deal_sales' || this.status == "ecos_oit_deal_resubmit" || this.status == '' || this.status == undefined || this.status == 'ecos_status_draft') && this.flag != '1') {
        foreignFrom.get("foreignTradeCorpSapCode").disable();
        foreignFrom.get("foreignTradeCorpDdpValidityDate").disable();
        foreignFrom.get("foreignTradeCorpTaxNum").disable();
      }
  ...
  isVatInfo(i) {
    ...
    const orderCurrencySystem = orderBaseinfo.getRawValue().currencySystem;
    const orderModality = orderBaseinfo.getRawValue().orderModality;
    if (currencySystem == 'USD' && orderCurrencySystem == 'CNY' && orderModality == 'PD&IGT') {
      ...
      accountFrom.get('invoicesDeliverAddress').setValidators(Validators.required)
  ...
  specialShow(i) {
    ...
    const { orderModality } = orderBaseinfo.getRawValue();
    const { oitMode, businessModel } = this.baseInfoFrom.getRawValue();
    if ((sampleCheck == '1') || (orderModality == "PD&IGT" && businessModel == 'DIRECT' && oitMode == 'BIDDING') || (orderModality == "US" && businessModel == 'DIRECT') || (orderModality == "CC" && businessModel == 'DIRECT')) {
      return true
    }
    else {
      return false
    ...
    if (orderModality == 'US') {
      if (includeSolution == '1') {
        ...
        marketBundleInfo.controls.forEach((val, index) => {
          ...
          group.get("businessOpportunityHierarchyLink").setValidators(Validators.required)
          group.get("businessOpportunityHierarchyLink").updateValueAndValidity();
          group.patchValue({
  ...
  segmentNzRequired(i) {
    ...
    if (orderSameEndUser == '0' && orderSales == this.user && this.flag != '1') {
  ...
  isRequireorderSalesPerformanceProvince(i) {
    ...
    const orderBaseinfo = this.orderInfo.at(i).get("orderBaseinfo") as FormGroup;
    const { orderModality } = orderBaseinfo.getRawValue();
    if (orderModality == 'US' && !orderSalesinfo.get("orderSalesPerformanceProvince").disabled) {
      orderSalesinfo.get("orderSalesPerformanceProvince").setValidators(Validators.required);
      orderSalesinfo.get("orderSalesPerformanceProvince").updateValueAndValidity();
      return true;
    }
    else {
  ...
  computContral(param)
  {
    ...
    let summary:any=0;
    const   orderBaseinfo = this.orderInfo.at(param.index).get("orderBaseinfo") as FormGroup;
    const   financialSolutionCnyNow=-floatSub(Number(param.financialSolution),Number(param.financialSolutionCp))
    const   creditCnyNow=floatSub(Number(param.credit),Number(param.creditCp))
    const   paymentNetCnyNow=floatSub(Number(param.payment),Number(param.paymentCp))
  ...
  {  
    this.computContral(param)
     const dealFinancialSolutionCnyNet=this.priceApproval.getRawValue()[param.value];
     const financialSolutionCountList=this.orderInfo.controls.map((item,index)=>{
          const orderBaseinfo=this.orderInfo.at(index).get("orderBaseinfo") as FormGroup;
          const financialSolutionCnyNet=orderBaseinfo.getRawValue()[param.value];
          return financialSolutionCnyNet
      }) 
      const totalAmount = financialSolutionCountList.reduce((a, b) =>floatAdd(Number(a),Number(b)));
