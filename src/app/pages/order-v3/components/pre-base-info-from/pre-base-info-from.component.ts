...
export class PreBaseInfoFromComponent implements OnInit {
  constructor(
    ...
    this.serveic.productReceive.subscribe((val) => {
      ...
      this.isaccountFrom();
      const { dealFormSalesProvince, businessModel,orderSalesProvince,dealFormSalesModality } =
  ...
  get financialSolutionNameModel() {
    const { financialSolutionName, financialSolutionOther } = this.priceApproval.getRawValue();
  ...
  init() {
    ...
    if ((this.status == 'ecos_oit_order_upload' && this.flag == '0') || this.needFileType == 'om') {
  ...
  isRequireorderSalesPerformanceProvince()
  {
    if(!this.isContract)
    {
      ...
      {
        this.baseInfoFrom.get("dealFormSalesPerformanceProvince").setValidators(Validators.required);
        this.baseInfoFrom.get("dealFormSalesPerformanceProvince").updateValueAndValidity();
        return true;
      }
      else{
        this.baseInfoFrom.get("dealFormSalesPerformanceProvince").clearValidators();
        this.baseInfoFrom.get("dealFormSalesPerformanceProvince").updateValueAndValidity();
        return false;
      }
    }
    else
    {
      ...
      {
        this.baseInfoFrom.get("orderSalesPerformanceProvince").setValidators(Validators.required);
        this.baseInfoFrom.get("orderSalesPerformanceProvince").updateValueAndValidity();
        return true;
      }
      else{
        this.baseInfoFrom.get("orderSalesPerformanceProvince").clearValidators();
        this.baseInfoFrom.get("orderSalesPerformanceProvince").updateValueAndValidity();
        return false;
      }
    }
  ...
  onShowSelectHospitalModal() {
    if (!this.iscontractBuyer) {
      const orderSalesModality = this.baseInfoFrom.getRawValue().dealFormSalesModality;
  ...
  initSaleRegions(role) {
    if (
      ...
      if (this.saleRegions.length == 1) {
        const { team, modality, cycleGroup, bigArea, smallArea, province, cluster, bmc, role } = regions[0];
        this.baseInfoFrom.patchValue({
  ...
  endUserRquired() {
    ...
    if ((dealFormSalesModality == "PD&IGT" && oitMode == "STOCK") || (dealFormSalesModality == "CC")) {
      ...
      this.endUserFrom.get("endUserActuallyDeliveryAddress").updateValueAndValidity();
      return false;
    } else {
      ...
      this.endUserFrom.get("endUserActuallyDeliveryAddress").setValidators(Validators.required);
  ...
  async onDealFormSelect(val) {
    ...
    this.baseInfoFrom.patchValue({
      ...
      id: id,
      approvalAreaConfiguration:
        this.saleRegions && this.saleRegions.length == 1 ? this.saleRegions[0].value : null,
      dealFormSalesModality: this.saleRegions && this.saleRegions.length == 1 ? this.saleRegions[0].modality : null,
      dealFormSalesTeam: this.saleRegions && this.saleRegions.length == 1 ? this.saleRegions[0].team : null,
      dealFormSalesBigArea: this.saleRegions && this.saleRegions.length == 1 ? this.saleRegions[0].bigArea : null,
      dealFormSalesCycleGroup: this.saleRegions && this.saleRegions.length == 1 ? this.saleRegions[0].cycleGroup : null,
      dealFormSalesSmallArea: this.saleRegions && this.saleRegions.length == 1 ? this.saleRegions[0].smallArea : null,
    });
    ...
    const dealData = await this.serveic.cpDealFormInfo(val.dealFormId)
      ...
      if (dealData.code == "0000") {
        ...
        this.baseInfoFrom.patchValue({
          ...
          //centralizedPurchasing:'0',
          businessModel,
          estimBiddingPrice: estimBiddingPrice != null && estimBiddingPrice != "" ? estimBiddingPrice : 0,
        ...
        if (dealerDMSData) {
          ...
          this.dealerFrom.patchValue({
            ...
            dealerCode: dealerDMSData.dealercode ? dealerDMSData.dealercode : dealerDMSData.dealeroldcodet,
        ...
        {
          this.endUserFrom.patchValue({
            endUserAddress:cdCustomerData.address?cdCustomerData.address:endUserAddress,
            endUserContact:cdCustomerData.customerContact?cdCustomerData.customerContact:endUserContact,
            endUserPhone:cdCustomerData.contactPhone?cdCustomerData.contactPhone:endUserPhone,
            ...
            hospitalType:cdCustomerData.customerType?cdCustomerData.customerType:hospitalType,
            endUser:cdCustomerData.customerName?cdCustomerData.customerName:hospitalName,
          });
        }
        else
        {
        ...
        this.priceApproval.patchValue({
          ...
          financialSolutionName:
            financialSolutionName && financialSolutionName != "null"
              ...
              : "",
          currencySystem,
          otherPaymentCnyNet:otherPaymentCnyNet!=null&&otherPaymentCnyNet!=""?otherPaymentCnyNet:0,
          otherPaymentCny:otherPaymentCny!=null&&otherPaymentCny!=""?otherPaymentCny:0,
          otherPaymentUsd:otherPaymentUsd!=null&&otherPaymentUsd!=""?otherPaymentUsd:0,
          financialSolutionCny: financialSolutionCnyNet != null && financialSolutionCnyNet != "" ? financialSolutionCnyNet : 0,
          financialSolutionCnyNet: financialSolutionCnyNet != null && financialSolutionCnyNet != "" ? financialSolutionCnyNet : 0,
          financialSolutionUsd: financialSolutionUsd != null && financialSolutionUsd != "" ? financialSolutionUsd : 0,
          dealPriceCny: equipmentPriceCny != null && equipmentPriceCny != "" ? equipmentPriceCny : 0,
          dealPriceCnyNet: equipmentPriceNetCny != null && equipmentPriceNetCny != "" ? equipmentPriceNetCny : 0,
          dealPriceUsd: equipmentPriceUsd != null && equipmentPriceUsd != "" ? equipmentPriceUsd : 0,
          equipmentPriceNetCny: equipmentPriceNetCny != null && equipmentPriceNetCny != "" ? equipmentPriceNetCny : 0,
          equipmentPriceCny: equipmentPriceCny != null && equipmentPriceCny != "" ? equipmentPriceCny : 0,
          //equipmentPriceCny:4000.23,
          equipmentPriceUsd: equipmentPriceUsd != null && equipmentPriceUsd != "" ? equipmentPriceUsd : 0,
          dealerSelfPurchasePriceNetCny: dealerSelfPurchasePriceNetCny != null && dealerSelfPurchasePriceNetCny != "" ? dealerSelfPurchasePriceNetCny : 0,
          dealerSelfPurchasePriceCny: dealerSelfPurchasePriceCny != null && dealerSelfPurchasePriceCny != "" ? dealerSelfPurchasePriceCny : 0,
          //dealerSelfPurchasePriceCny:1000.231,
          dealerSelfPurchasePriceUsd: dealerSelfPurchasePriceUsd != null && dealerSelfPurchasePriceUsd != "" ? dealerSelfPurchasePriceUsd : 0,
          vatRate,
        ...
        if (dealerDMSData && this.orderInfo && this.orderInfo.controls && this.orderInfo.controls.length > 0) {
          this.orderInfo.controls.forEach((val, index) => {
            const orderBaseinfo = this.orderInfo.at(index).get("orderBaseinfo") as FormGroup;
  ...
  checkBiddingEqualDealer(){
    ...
    if(biddingCompany && biddingCompany != "" && dealerName && subTierInfo && subTierInfo.length > 0){
      var dealerCompanyList = [dealerName];
      subTierInfo.filter(item => {
        if(item.dealerSubTiers && item.dealerSubTiers.length>0){
          dealerCompanyList = [...dealerCompanyList, ...item.dealerSubTiers.map(val => val.name)]
        }
      })
  ...
  importDealInform(param) { //导入经销商code
  ...
  selectPerformanceProvince(event) {
    ...
    if (!this.isContract && centralizedPurchasing == '0') {
      this.selectConfigCity('dealFormSalesPerformanceProvince', 'orderSalesPerformanceProvince')
    }
  }
      ...
      if (!this.isContract) {
        if (this.orderInfo && this.orderInfo.controls && this.orderInfo.controls.length > 0) {
          this.orderInfo.controls.map((vals, index) => {
            ...
            const {orderModality,currencySystem} = orderBaseinfo.getRawValue();
            ...
            {
              params.hospitalType=orderHospitalType!='集团'?orderHospitalType:""
            }
              ...
              if (paymentData.code == '0000') {
                ...
                if (data && data.length > 0) {
                  ...
                  if (!paymentProvision) {
                    ...
                    if(this.paymentTerm!=''&&this.paymentTerm!=undefined&&this.paymentTerm!=null)
                    {
                       paymentTermValue = payMentList.find(val => {
                        if (val.payment.replace(/\s*/g, "") == this.paymentTerm.replace(/\s*/g, "")) {
                          return val
                        }
                      })
                    }
                    ...
                    const paymentProvisionobj=payMentList.find(val=>{
                      if (val.payment.replace(/\s*/g, "") == paymentProvision.replace(/\s*/g, "")) {
                        return val
                      }
                    })
  ...
  isRequiredModality() {
    ...
    if (dealFormSalesModality == "PD&IGT" && oitMode == 'BIDDING') {
      ...
      this.baseInfoFrom.get("biddingCompany").setValidators(Validators.required);
  ...
  isRequiredArrivalDate() {
    //客户要货函货日期和预计安装日期是显示
    if (this.isContract) {
      const { dealFormSalesModality, orderModality, oitMode } = this.baseInfoFrom.getRawValue();
  ...
  isEstimateInstallationDate() {
    ...
    if (this.isContract) {
      const { dealFormSalesModality, orderModality, oitMode } = this.baseInfoFrom.getRawValue();
      if (orderModality == "US") {
        this.baseInfoFrom.get('requiredArrivalDate').setValidators(Validators.required)
        this.baseInfoFrom.get("estimateInstallationDate").setValidators(Validators.required);
        this.baseInfoFrom.get("estimateInstallationDate").updateValueAndValidity();
  ...
  preBookTitleHave() {
    if ((this.status == 'ecos_oit_order_upload' || this.needFileType == 'om') && this.baseInfoFrom.getRawValue().prebookQuantity > 1) {
      return true
    }
    else {
      return false;
  ...
  preBookTitleNothing() {
    if ((this.status == 'ecos_oit_order_upload' || this.needFileType == 'om') && this.baseInfoFrom.getRawValue().prebookQuantity == 0 && !this.baseInfoFrom.getRawValue().prebookReferenceId) {
      return true
    }
    else {
      return false;
  ...
  onShowSoluActualModal(i) {
    ...
    this.selectSoluUser.show({ modality: orderModality, role: 'Sales Rep/Mgr' }, true);
  }
  ...
  onShowSelectActualModal(i) {
    const { orderModality } = this.baseInfoFrom.getRawValue()
    this.selectActualUser.show({ modality: orderModality, role: 'Sales Rep/Mgr' }, true);
  }
  ...
  isshowActualSalesName() {
    ...
    if (this.isContract) {
      const { orderModality, centralizedPurchasing } = this.baseInfoFrom.getRawValue();
      if (centralizedPurchasing == '1') {
        this.baseInfoFrom.get("actualSalesEmail").setValidators(Validators.required);
  ...
  isSolutionSale() {
    if (this.isContract) {
      ...
      if (orderModality == 'PD&IGT' && host && host.businessOpportunityHierarchyLink) {
        return true;
      }
      else {
        return false;
