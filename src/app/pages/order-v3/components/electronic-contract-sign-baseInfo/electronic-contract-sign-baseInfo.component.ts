...
import { NumberThousandth, floatDivide, fomatFloat } from "@core/util/tools";
import * as moment from "moment";
@Component({
  selector: "ecos-electronic-contract-sign-baseInfo",
  templateUrl: "./electronic-contract-sign-baseInfo.component.html",
  styleUrls: ["./electronic-contract-sign-baseInfo.component.scss"],
})
export class ElectronicContractSignBaseInfoComponent implements OnInit {
  ...
  contractTemplate(item) {
    ...
    const { orderModality, businessModel } =
    ...
    marketBundleInfo.map((val) => {
      if (
        val.productModel != "" &&
        val.productModel != null &&
        val.productModel != undefined &&
    ...
    marketBundleInfo.map((items) => {
      ...
      if (
        ...
        if (
          items.newDealerAgreementNo != null &&
          items.newDealerAgreementNo != undefined &&
          items.newDealerAgreementNo != ""
        ) {
          dealerAgreementNo.push(items.newDealerAgreementNo);
        } else if (
          items.dtcDealerAgreementNo != null &&
          items.dtcDealerAgreementNo != undefined &&
          items.dtcDealerAgreementNo != null
        ) {
    ...
    const { tenderNum, biddingCompany, dealFormId, orderSales } =
      this.baseInfoFromData.getRawValue();
    let requiredArrivalDate =
      this.baseInfoFromData.getRawValue().requiredArrivalDate;
    let estimateInstallationDate =
      this.baseInfoFromData.getRawValue().estimateInstallationDate;
    const { foreignTradeCorpName, foreignTradeCorpAddress } =
      this.foreignFrom.getRawValue();

    const { totalContractPrice, dealPriceCny, dealPriceUsd, currencySystem } =
      this.priceApprovalData.getRawValue();

    estimateInstallationDate = estimateInstallationDate
      ? moment(estimateInstallationDate).format("YYYY年MM月DD日")
      : null;
    requiredArrivalDate = requiredArrivalDate
      ? moment(requiredArrivalDate).format("YYYY年MM月DD日")
      : null;

    if (orderModality == "US" || orderModality == "CC") {
      const tableParamsParam: any = this.generateTableParams(
        item,
        marketBundleInfo,
        totalContractPrice,
        priceTerms
    ...
    const params = {
      ...
      endUser: endUser,
      dealerCode:
        businessModel != "DIRECT" ? (dealerCode ? dealerCode : null) : null,
      ...
      totalContractPrice: totalContractPrice
        ? NumberThousandth(fomatFloat(totalContractPrice, 2))
        : "", //进单单位合同价
      totalPrice: totalPrice ? totalPrice : "", //表格总价
      contractPrice: contractPrice
        ? NumberThousandth(fomatFloat(contractPrice, 2))
        : "", //单价
  ...
  generateTableParams(
    item: any,
    marketBundleInfo: any,
    totalContractPrice: any,
    priceTerms
  ) {
    ...
    if (codeList.includes(item.code)) {
      ...
      marketBundleInfo.map((items, index) => {
        if (
          ...
          let unitPrice: any = floatDivide(
            totalContractPrice,
            items.marketBundleAmount
          );
          ...
          let arr = [
            ...
            {
              cellName:
                items.medicalDeviceName +
                " \n产品型号:" +
                items.productModel +
                (usIncludeMsg.includes(item.code)
                  ? " \n含相关的：以附件一配置清单为准"
                  : ""),
            },
            ...
            {
              cellName:
                (items.originCountry ? items.originCountry : "") + " +Philips",
            },
          ];
          ...
          if (codeColSix.includes(item.code)) {
            ...
            const arrCol = [
              { cellName: priceTerms ? priceTerms : "" },
              { cellName: unitPrice },
              { cellName: NumberThousandth(fomatFloat(totalContractPrice, 2)) },
            ];
            arr = [...arr, ...arrCol];
        ...
        if (
          ...
          if (ccCodeFive.includes(item.code)) {
            ...
            const arrCol = [
              {
                cellName:
                  (items.originCountry ? items.originCountry : "") +
                  " +Philips",
              },
              { cellName: items.marketBundleAmount },
              { cellName: "" },
  ...
  handleFileSigned({ signedFileId, unsignedFileId }) {
    const signedFileRelationList = this.signFileForm.get(
  ...
  handleFileDeleted({ signedFileId }) {
    ...
    const index = signedFileRelationList
      .getRawValue()
      .findIndex((item) => item.signedFileId === signedFileId);
    if (index > -1) {
      signedFileRelationList.removeAt(index);
    }
  ...
  handleSourceFileDeleted(sourceFileId) {
    ...
    const index = signedFileRelationList
      .getRawValue()
      .findIndex((item) => item.unsignedFileId === sourceFileId);
    if (index > -1) {
      signedFileRelationList.removeAt(index);
    }
    console.log("delete", this.signFileForm.getRawValue());
