...
@Component({
  selector: 'select-third-deal',
  styleUrls: ["./select-third-deal.scss"],
  templateUrl: './select-third-deal.html',
})
export class SelectThirdDealerComponent implements OnInit {
 ...
 */
  getTableData(resetPageNo = false) {
    ...
    this.http.post('/act/ecos/thirdParty/randomPick/manual/query', this.searchParams)
      .subscribe(({ code, data }) => {
        if (code === '0000') {
