...
@Component({
  selector: 'select-prebook',
  styleUrls: ["./select-prebook.component.scss"],
  templateUrl:'./select-prebook.component.html',
})
export class SelectPrebookComponent implements OnInit {

  @Output() select: EventEmitter<any> = new EventEmitter()
  visible: boolean = false
  tableData = DEFAULT_TABLE_DATA
  searchParams = DEFAULT_SEARCH_PARAMS
  searchByList: boolean = false
  mkbList: any[] = []
  ...
  query(resetPageNo = false){
    if(this.searchByList){
      this.getTableDataByList({...this.searchParams,marketBundleList:this.mkbList}, resetPageNo)
    }else{
      this.getTableData(resetPageNo)
    }
  }
   /**
   *
   ...
   */
    getTableData(resetPageNo = false) {
      ...
      this.http.post('/act/ecos/oit/findPrebookOrder', this.searchParams)
        .subscribe(({ code, data }) => {
          if (code === '0000') {
    ...
    showByList(params, loadData = false){
      this.visible = true
      if (loadData) {
        this.getTableDataByList(params)
      }
    }
    getTableDataByList(params,resetPageNo=false){
      this.searchByList = true
      if (resetPageNo) { params.pageNo = 1 }
      this.mkbList = params.marketBundleList || []
      this.tableData.loading = true
      this.http.post('/act/ecos/oit/findPrebookOrderByMarketBundleList', params)
        .subscribe(({ code, data }) => {
          if (code === '0000') {
            const { rows, total } = data
            this.tableData = { total, rows, loading: false }
