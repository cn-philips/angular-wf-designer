...
export class ExamineFormComponent implements OnInit {
  @Input() dataBase: any = {
    productList: [], // 产品列表
    detail: {
  ...
  ngOnInit(): void {
    ...
    this.dataBase.detail.taskID =
      this.activatedRouter.queryParams["_value"].taskID;
    ...
    this.validateForm = this.fb.group({
      ...
      paymentProvisionRadio: new FormControl(
    ...
    //DFBSH这种状态设置为必填项
    if (
      this.dataBase.detail.status === "DFBSH" &&
      this.dataBase.detail.taskID === "paymentProvision"
    ) {
      this.validateForm
        .get("paymentProvisionRadio")!
  ...
  isshowOrder() {
    if (
      (this.dataBase.detail.status == "cancel_oa_leader_approval" ||
        this.dataBase.detail.status == "cancel_sales_approval") &&
      !this.withdraw
    ) {
      return true;
    } else {
  ...
  submitForm = (value: any, check: number) => {
    ...
    switch (status) {
      // 销售部门一级审核
      case "DXSBMSH":
        url = "/act/ecom/order/application/checkSale";
        break;
      // 销售 二级部门审核
      case "DXSBM2JSH":
        url = "/act/preparation/secondaryDepartmentAudit";
        break;
      // 待非标审核
      case "DFBSH":
        url = "/act/preparation/secondaryAudit";
        break;
      // 特批文件进单初审（特批进单审核）
      case "DTPJDSH":
        url = "/act/preparation/specialFileReview";
        break;
      // 特批文件进单二审
      case "b2":
        url = "/act/preparation/specialFileReview";
      ...
      // 进单确认（部门审核）
      case "DOAJDQR":
        url = "/act/preparation/childOrderCheck";
        break;
      // OA审核
      case "DHTOASH":
        url = "/act/preparation/childOaReview";
        break;
