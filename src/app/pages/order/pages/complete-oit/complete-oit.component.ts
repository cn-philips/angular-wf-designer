...
export class CompleteOitComponent implements OnInit {
  ...
  @ViewChild("changeChild") changeChild;
  ...
  public allowChangOrder: any = false;
  //改单弹窗口
  ...
  public ngOnInit() {
    ...
    const ASYNS = async () => {
      ...
      const getUsers = await this.getUser();
      //const getOrderUserRegions=await this.getOrderUserRegion()
      const getOmUsers = await this.getOmUser();
    };
    ASYNS();
  }
  init() {
    ...
    this.allowChangOrder =
      this.activatedRouter.queryParams["_value"].allowChangOrder;
    switch (param) {
      case "third":
        this.thirdOff = true;
        this.disa = true;
  ...
  changeOrder() {
    ...
    this.http.get(url).subscribe((rest) => {
      if (rest.data) {
        this.isShow = true;
        this.changeData.id = this.mainId;
        this.ServesiceService.confirmTime.emit(this.changeData);
      } else {
    ...
    return new Promise((resolve, reject) => {
      this.http.post(url, {}).subscribe((res) => {
        if (res.data) {
          ...
          this.infor.detail.flag =
            this.activatedRouter.queryParams["_value"].flag;
          this.infor.detail.status =
    ...
    return new Promise((resolve, reject) => {
      this.http.get(url).subscribe((res) => {
        if (res.data) {
          ...
          this.dataBase.customerRequestLetter = formatDatesNow(
            this.dataBase.customerRequestLetter
          );
          this.dataBase.contractConfirmationDate = formatDatesNow(
            this.dataBase.contractConfirmationDate
          );
          resolve(res.data);
          ...
          this.dataBase.bidWinningPrice = res.data.bidWinningPrice
            ? res.data.bidWinningPrice
            : ""; //中标价格
          this.dataBase.relationshipLink =
          ...
          this.dataBase.countryOriginEn = params.countryOriginEn
            ? params.countryOriginEn
            : ""; // 原产地英文

          this.dataBase.medicalDeviceName = params.medicalDeviceName; //医疗器械名称
          this.dataBase.nmpaRegistrationExpried =
            params.nmpaRegistrationExpried; //NMPA证有效期截止日期
          this.dataBase.financialProgramme = param.financialProgramme; //金融方案价格
          this.dataBase.financialProgrammeTxt = param.financialProgrammeTxt; //金融方案文本框的值
          this.dataBase.tradeInCost = param.tradeInCost; //tradeIn总额
          this.dataBase.financialProgrammeCost = param.financialProgrammeCost; //金融方案总金额
          ...
          this.dataBase.isPrebookApply =
            param.isPrebookApply != null
              ...
              : "0"; //是否关联prebook
          this.dataBase.contractCancelReferenceId =
  ...
  public getFormDetails() {
    return new Promise((reslove, reject) => {
      this.http
        .get(
          `/act/preparation/oitCheck?mainId=` +
        ...
        .subscribe((res) => {
          if (res.code === "0000") {
            ...
            this.oitData.deBook = this.oitData.deBook
              ? this.oitData.deBook
              : "0";
            this.oitData.reBook = this.oitData.reBook
              ? this.oitData.reBook
              : "0";
            this.oitData.isCancel = this.oitData.isCancel
              ? this.oitData.isCancel
              : "0";
            this.oitData.exportControl = this.oitData.exportControl
              ? this.oitData.exportControl
              : "";
            this.oitData.other = this.oitData.other ? this.oitData.other : "";
            this.oitData.expertList = this.oitData.expertList
              ? this.oitData.expertList
              : [];
            !this.oitData.logisticsTime &&
              (this.oitData.logisticsTime = formatDatesNowMth(new Date()));
            this.oitData.specialApprovalSupporting =
              this.oitData.specialApprovalSupporting != null
                ? this.oitData.specialApprovalSupporting
                : "0";
            this.oitData.remark =
              this.oitData.remark != null ? this.oitData.remark : "";
  ...
  public BringOMLogisticianIsNull() {
    if (
      this.oitData &&
      this.oitData.logistician != null &&
      this.oitData.logistician !== ""
    ) {
      return;
    }
  ...
  getUser() {
    let marinId = decodeString(this.activatedRouter.queryParams["_value"].id);
    // let url = `/act/preparation/getOitExpert?mainId=${marinId}`;
    let url = "/act/role/getUsersByRoleAndModality?role=OM&modality=PD%26IGT";
    return new Promise((reslove, reject) => {
  ...
  async getOrderUserRegion() {
    let marinId = decodeString(this.activatedRouter.queryParams["_value"].id);
    const url = `/act/ecom/homepage/queryProcessOwner?mainId=${marinId}`;
  ...
  async getOmUser() {
    ...
    const param = {
      //initiatorBigArea:this.dataBase.region,
      initiatorBigArea: "",
      //initiatorSmallArea:this.dataBase.smallArea,
      initiatorSmallArea: "",
      initiatorTeam: "",
      //initiatorTeam:this.dataBase.team,
      initiatorEmail: this.sale,
    ...
    this.http.post(url, param).subscribe((res => {
      if (res.code == '0000') {
        const { data } = res;
        if (data && data.length > 0 && !this.oitData.logistician) {
          this.oitData.logistician = data[0].approverEmail
        }
      }
    }),
      (error) => {
  ...
  public getTemplate() {
    ...
    let additionalCondition =
      this.infor.businessModel == "DISTRIBUTOR" &&
        this.infor.invoiceInformation == "USD"
        ? this.infor.sameFlag
        : null;
    const param = {
        ...
        if (res.code === "0000") {
          if (res.data) {
            ...
            contractSignedAttachmentDTOList.map((vals) => {
              ...
              this.signingData.fileFileList =
                this.signingData.fileFileList.concat(obj);
            });
            resolve(this.signingData);
          }
        } else {
  ...
  submit(number: number, flag?: any) {
    ...
    this.oitData.mainId = decodeString(
      this.activatedRouter.queryParams["_value"].id
    );
    this.oitData.logisticsTime &&
      (this.oitData.logisticsTime = formatDatesNowMth(
        this.oitData.logisticsTime
      ));
    ...
    if (number == 1) {
      ...
      if (
        this.oitData.exportControl == "" ||
        this.oitData.exportControl == null ||
        this.oitData.exportControl == null
      ) {
        this.myskip("complete-padd");
      ...
      if (
        ...
        this.paymentProvision.indexOf(this.dataBase.paymentProvision) !== -1
      ) {
    ...
    if (this.oitData.logistician) {
      let usrArr = this.oitData.oMlist.find(
        (res) => this.oitData.logistician == res.email
      );
      let obj = {
        name: usrArr.name,
        userId: usrArr.id,
    ...
    this.load = true;
    this.http.post(url, this.oitData).subscribe(
      (res) => {
        this.load = false;
        if (res.code == "0000") {
  ...
  isChangeOk() {
    ...
    const processInstanceTaskId =
      this.activatedRouter.queryParams["_value"].processInstanceTaskId;
    this.changeData = this.changeChild.infor;
    let param = {
      mainId: this.mainId,
      check: this.changeChild.infor.refuseReason,
      orderChangeId: this.changeChild.infor.refuseReason,
      file: this.changeChild.infor.file,
      remark: this.changeChild.infor.remarks,
      processInstanceTaskId: processInstanceTaskId,
      changeDealForm: this.changeChild.infor.changeDealForm,
    };
    let vaild = this.changeChild.checkFormData();
    if (!vaild) {
      // this.message.create("error","有必填项没有填写")
      return;
    }
    ...
    this.http.post(url, param).subscribe(
      (rest) => {
        if (rest.code === "0000") {
          ...
          this.message.create("success", rest.msg);
          //this.myEvent.emit()
          this.changeChild.infor.file = "";
          this.changeChild.infor.refuseReason = null;
          this.changeChild.validateForm.reset();
          this.isShow = false;
          this.routerExtendService.back();
          // this.router.navigate(["ecos/my-started"]);
        } else {
