import { decodeString, formatDatesNow, standardTime } from "@core/util/tools";
...
export class PreordermodifComponent implements OnInit {
    ...
    return new Promise((resolve, reject) => {
      this.http.get(url).subscribe((res) => {
        if (res.data) {
          ...
          this.dataBase.productList = this.dataBase.productList
            ? this.dataBase.productList
            : [];
          this.dataBase.isPrebookApply =
            this.dataBase.isPrebookApply != null
              ? this.dataBase.isPrebookApply.toString()
              : "0";
          ...
          if (productList.length > 0) {
            productList.map((res) => {
              res.isPrebookApply =
                res.isPrebookApply != null
                  ? res.isPrebookApply.toString()
                  : "0";
              res.afterSales =
                res.afterSales != null &&
                res.afterSales != undefined &&
                res.afterSales != ""
                  ? res.afterSales
                  : "0";
          ...
          const filetake2 = [
            ...
            {
              id: "paymentProvisionFileName",
              name: "paymentProvisionFileNames",
            }, // 付款条件
            {
              id: "shipmentDeliveryFileName",
              name: "shipmentDeliveryFileNames",
            }, // 装运及交货
            { id: "sitePreparationFileName", name: "sitePreparationFileNames" }, // 场地准备
            {
              id: "installationWarrantyFileName",
              name: "installationWarrantyFileNames",
            }, // 安装，验收及保修
            {
              id: "amountDifferenceFileName",
              name: "amountDifferenceFileNames",
            }, // 直投订单合同金额和中标金额有价差
            { id: "performanceBondFileName", name: "performanceBondFileNames" }, // 履约保函
            { id: "afterSalesFileName", name: "afterSalesFileNames" }, // 是否售后
            {
              id: "supportFileMissingFileName",
              name: "supportFileMissingFileNames",
            }, // 支持文件缺失需特批进单
  ...
  getMarks() {
    ...
    this.load = true;
    this.http.post(`/act/process/getProcessWorkHisInfo`, param).subscribe(
      (rest) => {
        if (rest.code === "0000") {
          this.load = false;
  ...
  saveContract(): void {
    ...
    this.dataBase.processInstanceTaskId =
      ...
      this.dataBase.productList.map((res) => {
        ...
        if (res.productList.length > 0) {
          res.productList.map((val) => {
            val.modalityBmcs = val.modalityBmc
              ? val.modalityBmc
              : val.modalityBmcs;
            val.referenceId = "";
    ...
    this.dataBase.biddingPrice = this.dataBase.biddingPrice
      ? this.dataBase.biddingPrice
      : "0";
    this.load = true;
    this.http
      .post(`/act/preparation/saveAndSubmit`, this.dataBase)
      .subscribe((rest) => {
        if (rest.code === "0000") {
          ...
          if (rest.data) {
            ...
            this.dataBase.processInstanceTaskId =
              rest.data.processInstanceTaskId;
          }
          this.load = false;
        } else {
          this.message.create("error", `${rest.msg} ${rest.data}`);
          this.load = false;
        }
      });
  }
  ...
  public winningBid(): void {
    ...
    if (!(this.dataBase.productList && this.dataBase.productList.length > 0)) {
      ...
      for (let i = 0; i < this.dataBase.productList.length; i++) {
        ...
        let host = this.dataBase.productList[i].productList.find(
          (val) => val.checked
        );
        //是否有磁共震或者塔吊的验证
        if (host.modalityBmc) {
          ...
          if (bmcIgtDisble) {
            if (
              this.dataBase.productList[i].confirmationFileFlag === "0" &&
              (fi == null || fi === "")
            ) {
        ...
        //判断是否售后文件上传
        if (
          productList.afterSales == 1 &&
          (productList.afterSalesFileName == null ||
            productList.afterSalesFileName == "" ||
            productList.afterSalesFileName == undefined)
        ) {
        ...
        const foreignTradeCompanys = this.dataBase.foreignTradeCompany
        ...
        if (
          ...
          foreignTradeCompanys != distributors
        ) {
          if (
            ...
            this.message.create(
              "error",
              '外贸公司不在IE Pool！请重新从IE Pool选择外贸公司，或勾选"其它条款：进出口公司选择不在IE Pool"'
            );
            return;
          }
        }
        ...
        if (this.dataBase.businessModel == "DISTRIBUTOR") {
          if (
            productList.agreementNo == "" ||
            productList.agreementNo == undefined ||
            productList.agreementNo == null
          ) {
        ...
        if (this.dataBase.entryMode == "BIDDING" && this.dataBase.centralized) {
          ...
          if (
            productList.actualSales == "" ||
            productList.actualSales == undefined ||
            productList.actualSales == null
          ) {
        ...
        //提示付款条款
        if (
          productList.paymentProvision == "" ||
          productList.paymentProvision == undefined ||
          productList.paymentProvision == null
        ) {
        ...
        if (
          [
            "36b88bf6-1864-11ec-9074-54ee75a9b10b",
            ...
            "36b9c34f-1864-11ec-9074-54ee75a9b10b",
          ].indexOf(productList.paymentProvision) !== -1
        ) {
          // 付款条款选择“其他”时，备注不能为空
        ...
        // 装运及交货
        if (
          productList.shipmentDelivery === "1" ||
          productList.shipmentDelivery === 1
        ) {
        ...
        // 安装，验收及保修
        if (
          productList.installationWarranty === "1" ||
          productList.installationWarranty === 1
        ) {
        ...
        // 直投订单合同金额和中标金额有价差
        if (
          productList.amountDifference === "1" ||
          productList.amountDifference === 1
        ) {
        ...
        // 支持文件缺失需特批进单
        if (
          productList.supportFileMissing === "1" ||
          productList.supportFileMissing === 1
        ) {
    ...
    if (
      ...
      const tenderingCompany = this.dataBase.tenderingCompany
        ? this.dataBase.tenderingCompany.replace(/\s+/g, "")
        : "";
     ...
     */
    sampleAuditFlagArr = this.dataBase.productList.every(
    ...
    if (this.dataBase.entryMode == "STOCK") {
      ...
      const userTeamOne =
        this.dataBase.userTeme == "VAD" ||
        this.dataBase.userTeme == "CT VAD" ||
        this.dataBase.userTeme == "CTVAD";
      const userTeamTwo =
        this.dataBase.userTeme == "VAD" ||
        this.dataBase.userTeme == "BV" ||
        this.dataBase.userTeme == "DXR" ||
        this.dataBase.userTeme == "CT VAD" ||
        this.dataBase.userTeme == "CTVAD";
      if (
        (this.dataBase.bidWinningNotice == "" ||
          this.dataBase.bidWinningNotice == null ||
          this.dataBase.bidWinningNotice == undefined) &&
        userTeamOne
      ) {
        let title =
          this.dataBase.tenderNo != "其他类型" ? "中标通知书" : "最终用户合同";
        this.myskip("pending-tab");
      ...
      if (
        (this.dataBase.siteReport == "" ||
          this.dataBase.siteReport == null ||
          this.dataBase.siteReport == undefined) &&
        userTeamTwo
      ) {
        let demandLetter;
        if (
    ...
    this.dataBase.productList.map((res) => {
      ...
      obj.appPerson = localStorage.getItem("ecom_ng_philips_code1");
    ...
    if (arrIscheak.length > 0 && marketBundLen.length == this.dataBase.count) {
      ...
      this.tableLoad = true;
      this.http.post(url, arrIscheak).subscribe(
        (res) => {
          if (res.code === "0000") {
            this.tableLoad = false;
            if (res.data.length > 0) {
              this.dataBase.biddingPrice = res.data[0].biddingPrice
                ? res.data[0].biddingPrice
                : 0;
  ...
  handleOkWinCheck(): void {
    ...
    this.dataBase.productList.map((res) => {
      //中标效验成功的referenceId,productionInformId 赋值给对应的产品
      res.checked = false;
      res.productList.map((vals) => {
        vals.configurationFileList = vals.configurationFiles
          ? vals.configurationFiles
          : [];
    ...
    const ASYNS = async () => {
      ...
      if (
        applying.length > 0 &&
        applying[0].taskStatus != "prebook_end" &&
        !this.isSubmit
      ) {
      ...
      if (
        ...
        if (
          ...
          this.dataBase.preBook.foreignTradeCompany
        ) {
          this.message.create(
            "warning",
            "外贸公司修改后slot可能被取消，请考虑后提交"
          );
        }
      }
      if (
  ...
  takeFile(filename, filename2) {
    ...
    for (let i = 0; i < this.dataBase.productList.length; i++) {
      ...
      if (
        this.dataBase.productList[i][filename2.id] &&
        this.dataBase.productList[i][filename2.id] !== "" &&
        this.dataBase.productList[i][filename2.id] != undefined
      ) {
