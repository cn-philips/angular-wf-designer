...
import { decodeString, formatDatesNowMth, formatDatesNow, standardTime } from '@core/util/tools';
...
export class SuppfileComponent implements OnInit {
          ...
          if (res.code === "0000") {
            ...
            this.oitData.specialApprovalSupporting =
              this.oitData.specialApprovalSupporting != null
                ? this.oitData.specialApprovalSupporting
                : "0";
            this.oitData.specialSupportCompleted =
              this.oitData.specialSupportCompleted != null
                ? this.oitData.specialSupportCompleted
                : "0";
            reslove(res.data);
            let specialApprovalDocumentsName =
              this.oitData.supportFileMissingFileName;
            if (
              specialApprovalDocumentsName != null &&
              specialApprovalDocumentsName != undefined &&
              specialApprovalDocumentsName != ""
            ) {
              specialApprovalDocumentsName =
                specialApprovalDocumentsName.split(",");
  ...
  save(params) {
    ...
    if (params == 1) {
      if (
        (this.infor.tenderDocuments == "" ||
          this.infor.tenderDocuments == undefined ||
          this.infor.tenderDocuments == null) &&
      ...
      if (
        (this.infor.biddingDocuments == "" ||
          this.infor.biddingDocuments == undefined ||
          this.infor.biddingDocuments == null) &&
        this.infor.tenderNo != "其他类型"
      ) {
      ...
      if (
        (this.infor.endUserContract == "" ||
          this.infor.endUserContract == undefined ||
          this.infor.endUserContract == null) &&
        this.infor.businessModel == "DISTRIBUTOR"
      ) {
      ...
      if (
        (this.infor.projectAnalysisTable == "" ||
          this.infor.projectAnalysisTable == undefined ||
          this.infor.projectAnalysisTable == null) &&
        this.infor.businessModel == "DISTRIBUTOR"
      ) {
    ...
    this.param.mainId = decodeString(
      this.activatedRouter.queryParams["_value"].id
    );
    this.param.processInstanceTaskId = processInstanceTaskId;
    this.http.post(url, Object.assign(this.infor, this.param)).subscribe(
      (res) => {
        if (res.code === "0000") {
          this.message.create("success", `${res.msg}`);
          if (params === "1" || params === 1) {
            // this.router.navigate(["/ecos/oit-supplement"]);
  ...
  isChangeOk() {
    const processInstanceTaskId =
      this.activatedRouter.queryParams["_value"].processInstanceTaskId;
    this.showData = this.changeChild.infor;
    let param = {
      mainId: this.mainId,
