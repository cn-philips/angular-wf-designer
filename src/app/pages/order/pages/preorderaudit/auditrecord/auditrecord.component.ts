...
export class AuditrecordComponent implements OnInit {
  ...
  ngOnChanges() {
    ...
    this.dataBase.tableColOff = roles.some(
      (val) =>
        val == "OA" ||
        val == "OA Leader" ||
        val == "CFC Leader" ||
  ...
  ngOnInit() {
    ...
    this.title =
      this.activeRoute.queryParams["_value"].state == "DOACS"
        ? "Review the Order Preparation Form 审核进单准备表"
        : "Review the Contract Summary 审核合同概要表";
    this.validateForm = this.fb.group({
      remark: new FormControl(
        { value: "", disabled: this.flag == 1 },
        Validators.required
      ),
    });
  }
  ...
  public tipsfun() {
    if (
      ...
      this.dataBase.isSame == "1" &&
      this.dataBase.tableColOff
    ) {
      return true;
    } else {
      return false;
    ...
    if (this.dataBase.productList && this.dataBase.productList.length > 0) {
      if (e == 1) {
        ...
        for (let i = 0; i < this.dataBase.productList.length; i++) {
          ...
          const afterSalesFileName =
            this.dataBase.productList[i].afterSalesFileName;
          ...
          if (this.dataBase.isVerify) {
            if (
              ...
              this.dataBase.supportingFile == null ||
              this.dataBase.supportingFile == undefined
            ) {
          ...
          if (afterSales == "1") {
            if (
              afterSalesFileName == "" ||
              afterSalesFileName == null ||
              afterSalesFileName == undefined
            ) {
          ...
          const totalContractPrice =
            this.dataBase.productList[i].totalContractPrice;
          ...
          if (
            (this.dataBase.financialProgramme == "1" ||
              this.dataBase.financialProgramme == "2") &&
            this.dataBase.financialProgrammeCost != 0.0
          ) {
            const financialPrice = this.dataBase.productList[i].financialPrice;
            if (
              financialPrice == null ||
              financialPrice == "" ||
              financialPrice == undefined
            ) {
          ...
          summary =
            (summary * 100 +
              this.dataBase.productList[i].financialPrice * 100) /
            100;
        }
        let diffs = Math.abs(
          (this.dataBase.financialProgrammeCost * 100 - summary * 100) / 100
        ); //金融方案总价和明细价格对比
        let difference = Math.abs(
          (this.dataBase.entryUnitPrice * 100 -
            this.dataBase.dealContractPrice * 100) /
            100
        ); //Deal Form总价和E-COM总价对比
        ...
        if (!this.dataBase.isVerify) {
          if (this.dataBase.invoiceInformation == "CNY") {
            ...
            let dealContractPrice =
              this.dataBase.dealContractPrice * (1 + taxrate);
            ...
            if (
              summay > dealContractPriceadd ||
              summay < dealContractPricereduce
            ) {
              this.message.create(
                "error",
                `进单单位合同总价与Deal Form总价价差不能>${len}`
              );
              return false;
            }
          } else {
          ...
          if (this.dataBase.invoiceInformation == "CNY") {
            ...
            for (let i = 0; i < productList.length; i++) {
              summay =
                (summay * 100 + productList[i].totalContractPrice * 100) / 100;
            }
            console.log(summay);
            // this.dataBase.productList.map((a)=>{
            //   summay = (summay * 100 + a.totalContractPrice*100) / 100;
      ...
      this.dataBase.productList.map((res) => {
        let obj = {
          id: res.id,
          installationWarrantyRadio: res.installationWarrantyRadio
            ? res.installationWarrantyRadio
            : "",
        };
