...
import { formatDatesNow, standardTime } from "@core/util/tools";
...
export class PreOrderComponent implements OnInit {
  ...
  public saveContract(): void {
    ...
    this.dataBase.biddingPrice = this.dataBase.biddingPrice
      ...
      : "0";
    this.load = true;
    this.http.post(`/act/preparation/saveAndSubmit`, this.dataBase).subscribe(
      (rest) => {
        if (rest.code === "0000") {
  ...
  public winningBid(): void {
    ...
    if (!(this.dataBase.productList && this.dataBase.productList.length > 0)) {
      ...
      for (let i = 0; i < this.dataBase.productList.length; i++) {
        ...
        for (
          let j = 0;
          j < this.dataBase.productList[i].productList.length;
          j++
        ) {
          let productExpert =
            this.dataBase.productList[i].productList[j].productExpert;
          if (
            productExpert == null ||
            productExpert == "" ||
            productExpert == undefined
          ) {
        ...
        if (
          ...
          (productList.freeText == null ||
            ...
            productList.freeText == undefined)
        ) {
  ...
  public handleOkWinCheck(): void {
    ...
    const ASYNS = async () => {
      ...
      const result = await this.http.post(url, this.dataBase).subscribe(
        (rest) => {
          if (rest.code === "0000") {
            this.message.create("success", `${rest.msg}`);
            this.load = false;
  ...
  handleImporting(isLoading) {
    setTimeout(() => {
      this.nzLoading = isLoading;
    });
  }
}
