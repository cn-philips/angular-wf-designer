...
import { HttpService, FileService, ServesiceService } from '@core/services';
import { decodeString, formatDatesNow, NumberThousandth, standardTime, isadopt } from '@core/util/tools';
...
export class InorderComponent implements OnInit {
      ...
      this.http.post(url).subscribe((res) => {
        if (res.data) {
          ...
          this.dataBase.financialProgrammeCost =
            res.data.financialProgrammeCost; //金融方案总金额
          ...
          this.dataBase.isPrebookApply =
            res.data.isPrebookApply != null
              ? res.data.isPrebookApply.toString()
              : "0"; //是否关联prebook
          this.dataBase.prebookReferenceId = res.data.prebookReferenceId; //prebookid
          this.dataBase.prebookProductId = res.data.prebookProductId; //prebookProductId
          this.dataBase.prebookMainId = res.data.prebookMainId; //prebookMainId
          this.dataBase.priceDifferent = res.data.priceDifferent; //合cp价格是否一至
          this.dataBase.isVerify = res.data.isVerify; //是否一样
          this.dataBase.supportingFile = res.data.supportingFile; //上传支持文件
          this.dataBase.supportingFileNames = res.data.supportingFileNames; //支持文件名称
          this.infor.isPrebookApply = this.dataBase.isPrebookApply;
          this.dataBase.contractCancelReferenceId =
            res.data.contractCancelReferenceId;
          this.dataBase.contractCancelMainId = res.data.contractCancelMainId;
          this.dataBase.preparationProductCompany =
            res.data.preparationProductCompany;
          ...
          this.infor.detail.status =
            this.activatedRouter.queryParams["_value"].state;
  ...
  getQuery() { //查询order summary
    ...
    return new Promise((resolve, reject) => {
      this.http.get(url).subscribe((res) => {
        ...
        this.dataBase.isRecycle =
          this.dataBase.isRecycle != null ? this.dataBase.isRecycle : "0";
        resolve(this.addoff);
      });
    });
  }
  getBase() { //查询基础数据
    ...
    return new Promise((resolve, reject) => {
      this.http.get(url).subscribe((res) => {
        ...
        this.dataBase.bidWinningPrice = res.data.biddingPrice
          ? res.data.biddingPrice
          : "";
        this.dataBase.isUsdOrRmb =
          ...
          res.data.isUsdOrRmb != undefined
            ? res.data.isUsdOrRmb
            : "";
        this.dataBase.promotionPlan =
          res.data.promotions != "" ? res.data.promotions : ""; //促销计划
        this.dataBase.incentiveScheme =
          res.data.rebates != "" ? res.data.rebates : ""; //经销商奖励计划
        this.dataBase.specialApprovalDocuments = res.data.supportFileMissing; //特批文件后补
        this.dataBase.specialApprovalDocumentsName =
    ...
    this.http.get(url).subscribe((rest) => {
      if (rest.data) {
        this.dataBase.relationshipLink =
          rest.data.businessOpportunityHierarchyLink; // 商机层级关系链接
        this.dataBase.sofonFile = rest.data.sofonFile; //sofonFile
        // this.dataBase.sofonFileNames = rest.data.sofonFileNames;//名字
        this.dataBase.countryOrigin = rest.data.countryOrigin
          ? rest.data.countryOrigin
          : ""; // 原产地中文
        this.dataBase.medicalDeviceName = rest.data.medicalDeviceName; //医疗器械名称
        this.dataBase.nmpaRegistrationExpried =
          rest.data.nmpaRegistrationExpried; //NMPA证有效期截止日期
        this.dataBase.countryOriginEn = rest.data.countryOriginEn
          ? rest.data.countryOriginEn
          : ""; //原产地英文
        // this.dataBase.finalSofonQuotation=rest.data.sofonNo //finalSofonQuotation
        this.dataBase.tradeList =
          rest.data.cosOppTradeIns != null &&
          rest.data.cosOppTradeIns.length > 0
            ? rest.data.cosOppTradeIns
            : [{ name: "", costs1: "" }]; // tradeIn
        this.dataBase.warrantyList =
          rest.data.cosOppExtendedWarranties != null &&
          rest.data.cosOppExtendedWarranties != "" &&
          rest.data.cosOppExtendedWarranties.length > 0
            ? rest.data.cosOppExtendedWarranties
            : []; // 延长保修
        this.dataBase.productList =
          rest.data.cosOppThirdParties != null &&
          rest.data.cosOppThirdParties != "" &&
          rest.data.cosOppThirdParties.length > 0
            ? rest.data.cosOppThirdParties
            : []; // 第三方
        this.dataBase.otherList =
          rest.data.otherList != null &&
          rest.data.otherList != "" &&
          rest.data.otherList.length > 0
            ? rest.data.otherList
            : []; //其他预留
        ...
        this.dataBase.applications =
          rest.data.applications != null &&
          rest.data.applications != "" &&
          rest.data.applications.length > 0
            ? rest.data.applications
  ...
  submit(number: number, flag?: any) {
    ...
    this.dataBase.customerRequestLetter = customerDate
      ? formatDatesNow(customerDate)
      : "";
    this.dataBase.contractConfirmationDate = contractDate
      ? formatDatesNow(contractDate)
      : "";
    this.dataBase.contractSummaryId = decodeString(
    ...
    if (number == 1 && flag == "APPROVED") {
      ...
      if (
        this.infor.businessModel != "DIRECT" ||
        (this.infor.businessModel == "DIRECT" &&
          this.infor.tenderNo != "其他类型")
      ) {
        if (this.dataBase.bidWinningNoticeCheckFlag != "1") {
          let title =
            this.infor.tenderNo != "其他类型" ? "中标通知书" : "最终用户合同";
      ...
      if (this.dataBase.siteReportCheckFlag != "1") {
        let demandLetter;
        if (
          this.infor.entryMode == "STOCK" &&
          (this.infor.userTeme == "BV" || this.infor.userTeme == "DXR")
        ) {
          demandLetter = "要货函";
        } else {
      ...
      if (
        this.infor.entryMode == "BIDDING" ||
        (this.infor.sampleAuditFlag == "1" && this.infor.entryMode == "STOCK")
      ) {
      ...
      if (
        this.infor.mrShieldingCompany != null &&
        this.infor.mrShieldingCompany != undefined &&
        this.infor.mrShieldingCompany != ""
      ) {
      ...
      if (
        this.infor.confirmationFile != null &&
        this.infor.confirmationFile != undefined &&
        this.infor.confirmationFile != ""
      ) {
  ...
  backContract() {
    ...
    let obj = {
      ...
      code: "backContract",
      refuseReason: null,
      remarks: this.dataBase.remarks,
      file: this.dataBase.enclosure,
