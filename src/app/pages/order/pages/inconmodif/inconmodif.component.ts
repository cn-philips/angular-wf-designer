...
import { decodeString, getType, formatDatesNow, standardTime, isadopt } from '@core/util/tools';
...
export class InconmodifComponent implements OnInit {
  ...
  isVisibleDate: any = false;
  isVisibleDateIepool: any = false;
    ...
    this.http.post(url).subscribe((rest) => {
      if (rest.code === "0000") {
        if (rest.data) {
          ...
          this.params.remarks = this.dataBase.remarks
            ? this.dataBase.remarks
            : "";
          this.params.fileUpload = this.dataBase.fileUpload
            ? this.dataBase.fileUpload
            : "";
          ...
          const ASYNS = async () => {
            ...
            //改单中的合同概要表和正常的合同概要表
            if (
              this.dataBase.hsId != null &&
              this.dataBase.hsId != undefined &&
              this.dataBase.hsId != ""
            ) {
              this.getCheckhsId();
            } else {
  ...
  save(e) {
    ...
    // 获取mainId
    const mainId = decodeString(this.activatedRouter.queryParams["_value"].id);
    this.params.processInstanceTaskId =
    ...
    if (e == 1) {
      ...
      if (this.dataBase.other7 == true) {
        if (
          ...
          this.dataBase.freeText == null ||
          this.dataBase.freeText == undefined
        ) {
      ...
      if (this.dataBase.afterSales == "1") {
        if (
          ...
          this.dataBase.afterSalesFileName == null ||
          this.dataBase.afterSalesFileName == undefined
        ) {
      ...
      if (
        this.dataBase.supportFileMissing == "0" &&
      ...
      if (this.dataBase.confirmationFileFlags) {
        if (
          ...
          this.dataBase.confirmationFile == null
        ) {
      ...
      if (this.dataBase.mrShieldingCompanyFlags) {
        if (
          ...
          this.dataBase.mrShieldingCompany == null
        ) {
    ...
    const processInstanceTaskId =
      this.activatedRouter.queryParams["_value"].processInstanceTaskId;
    parm.processInstanceTaskId = processInstanceTaskId;
    const ASYNS = async () => {
      this.load = true;
      if (e == 1) {
        if (this.dataBase.businessModel == "DISTRIBUTOR") {
        ...
        if (parm.isPrebookApply == "0") {
          parm.prebookProductId = "";
          parm.prebookReferenceId = "";
          parm.prebookMainId = "";
        }
      }
  ...
  isAgregentOk() {
    ...
    parm.processInstanceTaskId = processInstanceTaskId;
    const ASYNS = async () => {
      let checkOit: any = false;
      // if (this.dataBase.hsId == null || this.dataBase.hsId == '') {
      //   checkOit = await this.getCheck();
      // }
      // if (!this.dataBase.hsId && !checkOit) {
      //   this.message.create('error', '已经有oit完成的进单不能关闭进单');
      //   return;
      // }
      this.load = true;
      this.http.post(url, parm).subscribe(
