...
import { codeString, decodeString } from '@core/util/tools';
...
export class InorderInComponent implements OnInit {
  ...
  constructor(private activatedRouter: ActivatedRoute, private fb: FormBuilder, private http: HttpService, private message: NzMessageService, private ServesiceService: ServesiceService,) { }
  ...
  //跳转到prebook链接
  public gotoWin(item) {
    console.log(location.origin + environment.base_href + '/#/' + 'pre-book/prebook-so?id=' + codeString(item) + '&flag=1');
    window.open(location.origin + environment.base_href + '/#/' + 'pre-book/prebook-so?id=' + codeString(item) + '&flag=1&status=prebook_end');
  }
  //经销用户列表
  distributorLoad(val) {
    let params: any = {
    ...
    if (params.dictGroup != '') {
      this.http.post(`/act/ecom/dictData/queryGroupDictData`, params).subscribe((rest => {
        if (rest.code === '0000') {
          ...
          //   select && (this.dataBase.paymentmethods = select.dictId);
          // }
        }
      }), (error => {
        this.message.create("error", "请求异常");
  ...
  ngOnChanges() {
    ...
    if (this.dataBase) {
      ...
      let distributor=this.dataBase.agent?this.dataBase.agent.replace(/\s+/g, ""):null
      distributor&&this.distributorLoad(distributor)
      this.selectFinacial();
      this.getPoolList();
      this.paymentMethod();
