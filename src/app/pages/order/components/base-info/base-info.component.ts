...
import { codeString, isadopt } from "@core/util/tools";
...
export class BaseInfoComponent implements OnInit {
  ...
  constructor(
    ...
    this.ServesiceService.supportFileMissing.subscribe((res) => {
      if (
        ...
        const miss = this.dataBase.productList.every(
          (vals) => vals.supportFileMissing == "1"
        );
    ...
    this.ServesiceService.prebook.subscribe((res) => {
      if (
        ...
        let prebookMainId = this.dataBase.productList.find(
          (val) =>
            val.prebookMainId != null &&
            val.prebookMainId != "" &&
            val.prebookMainId != undefined
        );
        if (prebookMainId) {
          const prebook = this.dataBase.productList.some(
            (val) => val.isPrebookApply == "1"
          );
  ...
  distributoBlur() {
    ...
    if (this.dataBase.productList && this.dataBase.productList.length > 0) {
      let productList = this.dataBase.productList.find(
        (val) =>
          val.prebookMainId != null &&
          val.prebookMainId != "" &&
          val.prebookMainId != undefined
  ...
  public prebookChange(e) {
    if (
      (e == "1" && this.dataBase.detail.status == "DHTGYBTX") ||
      (e == "1" && this.dataBase.detail.status == "XJDHTGYBTX") ||
      (e == "1" && this.dataBase.detail.status == "XJDHTGYBTX")
    ) {
      this.validateForm
        .get("prebookReferenceId")!
  ...
  centralizedChange(param) {
    ...
    if (
      param &&
      (status == "" || status == "DHTGYBTX" || status == "XJDHTGYBTX")
    ) {
  ...
  changePayment(state) {
    ...
    if (
      tenderPriceCurrencys == null ||
      tenderPriceCurrencys == undefined ||
      tenderPriceCurrencys == ""
    ) {
      this.dataBase.paymentList = [];
    ...
    if (this.dataBase.paymentList && this.dataBase.paymentList.length > 0) {
      let selectId = this.dataBase.paymentList.find(
        (val) => val.dictId == this.dataBase.paymentProvision
      );
  ...
  public paymentMethod() {
    ...
    applyTypeoff =
      this.dataBase.entryMode == "BIDDING" || this.dataBase.entryMode == "STOCK"
        ? true
        : false;
    let applyType = this.dataBase.entryMode;
    let clientType = this.dataBase.hospitalNature;
    ...
    if (applyTypeoff && clientType && tenderPriceCurrencys && businessType) {
      if (
        (applyType == "BIDDING"||applyType == "bidding_authorization") &&
        businessType == "DIRECT" &&
        tenderPriceCurrencys == "CNY" &&
        clientType == "公立医院"
      ) {
    ...
    if (params.dictGroup != "") {
      this.http.post(`/act/ecom/dictData/queryGroupDictData`, params).subscribe(
        (rest) => {
          if (rest.code === "0000") {
            this.dataBase.paymentList = rest.data;
            if (
              this.dataBase.paymentProvision == "0" ||
              this.dataBase.paymentProvision == "1"
            ) {
              let selectId = this.dataBase.paymentList.find(
                (val) => val.remark == this.dataBase.paymentProvision
              );
              this.dataBase.paymentProvision = selectId.dictId;
            }
            const ASYNS = async () => {
              if (this.dataBase.paymentmethod) {
                let paymentmethod = this.dataBase.paymentList.find(
                  (val) => val.dictLabel == this.dataBase.paymentmethod
                );
                if (paymentmethod) {
                ...
                if (this.edit && this.dataBase.dealFormId) {
                  ...
                  this.dataBase.paymentmethod =
                    dealFrom.paymentMethodDescription;
                  let paymentmethod = this.dataBase.paymentList.find(
                    (val) => val.dictLabel == this.dataBase.paymentmethod
                  );
                  if (paymentmethod) {
            ...
            if (selectId) {
              ...
              this.paymentOff &&
                status === "DHTOASH" &&
                flag == "0" &&
                this.validateForm.controls.paymentProvision.enable();
            } else {
              this.paymentOff = false;
  ...
  Tips() {
    if (
      this.taskId == "paymentProvision" &&
      this.paymentOff &&
      this.dataBase.detail.flag == "0"
    ) {
      return true;
    } else if (
      this.taskId == "performanceBond" &&
      this.dataBase.performanceBond == "1" &&
      this.dataBase.detail.flag == "0"
    ) {
      return true;
    } else if (
      this.taskId == "sitePreparation" &&
      this.dataBase.sitePreparation == "1" &&
      this.dataBase.detail.flag == "0"
    ) {
      return true;
    } else if (
      this.taskId == "installationWarranty" &&
      this.dataBase.installationWarranty == "1" &&
      this.dataBase.detail.flag == "0"
    ) {
      return true;
    } else if (
      this.taskId == "shipmentDelivery" &&
      this.dataBase.shipmentDelivery == "1" &&
      this.dataBase.detail.flag == "0"
    ) {
      return true;
    } else {
  ...
  Tipsecond() {
    if (
      ((this.taskId == "TPWJJDCS" ||
        this.taskId == "TPWJJDSH" ||
        this.taskId == "TPWJJDZS") &&
        this.dataBase.supportFileMissing == 1) ||
      ((this.taskId == "TPWJJDCS" ||
        this.taskId == "TPWJJDSH" ||
        this.taskId == "TPWJJDZS") &&
        this.dataBase.amountDifference == 1 &&
        this.dataBase.detail.flag == "0")
    ) {
      return true;
    } else {
  ...
  public generateAnalysisTemplate(code) {
    ...
    const params = {
      ...
      tenderingCompany: this.dataBase.tenderingCompany
        ? this.dataBase.tenderingCompany
        : "",
      dealFormId: this.dataBase.dealFormId,
    };
    this.pdfSRC = params;
    if (
      this.dataBase.dealFormId == "" ||
      this.dataBase.dealFormId == null ||
      this.dataBase.dealFormId == undefined
    ) {
      this.message.create("error", "请先查询dealFormId");
      return;
    }
    if (
      this.dataBase.tenderNo == "" ||
      this.dataBase.tenderNo == undefined ||
      this.dataBase.tenderNo == null
    ) {
    ...
    if (
      this.dataBase.tenderingCompany == "" ||
      this.dataBase.tenderingCompany == undefined ||
      this.dataBase.tenderingCompany == null
    ) {
  ...
  public setColSpanOfConfirmTable(database?: any): void {
    try {
      ...
      if (
        NewDatabase.sampleAuditFlag &&
        NewDatabase.sampleAuditFlag.toString() == "1"
      ) {
  ...
  public installationWarrantyBeforeUpload = (file: UploadFile): boolean => {
    ...
    this.upload(
      "installationWarrantyList",
      file,
      "installationWarrantyFileName"
    );
    return false;
  };
  //装运及交货上传
  ...
  public paymentProvisionBeforeUpload = (file: UploadFile): boolean => {
    ...
    this.upload(
      "paymentProvisionFileNameList",
      file,
      "paymentProvisionFileName"
    );
    return false;
  };
  // 上传盖章后的投标申请函
  ...
  setDatabase(data) {
    ...
    if (this.dataBase.invoiceInformation === "USD") {
      ...
      this.dataBase.foreignTradeCompanyAddress =
        data.foreignTradeCompanyAddress; //外贸公司
      this.dataBase.foreignTradeCompanyContacts =
        data.foreignTradeCompanyContacts; //外贸公司联系人
  ...
  public getCPDetails() {
    ...
    return new Promise((resolve, reject) => {
      this.http
        .post(`/act/preparation/queryCpReview`, {
          dealFormID: this.dataBase.dealFormId,
        })
        .subscribe((rest) => {
          if (rest.code === "0000") {
            ...
            if (
              ...
              this.dataBase.tenderingCompany =
                cosOppDealForm.biddingCompanyName; //投标公司
              ...
              this.dataBase.contractBuyerAddress =
                cosOppDealForm.registeredAddress; //合同买方地址
              ...
              if (
                ...
                this.dataBase.contractBuyerAddress =
                  this.dataBase.endUserAddress;
                this.dataBase.contractBuyerContacts =
                  this.dataBase.endUserContacts;
                ...
                this.dataBase.contractBuyerPhone =
                  this.dataBase.distributorPhone;
                this.dataBase.contractBuyerEmail =
                  this.dataBase.distributorEmail;
  ...
  isDistributor(data) {
    if (this.dataBase.isPrebookApply == "1") {
      ...
      this.distributorOffPrebook =
        distributor == distributorPrebook ? false : true;
    }
  }
      ...
      !this.dataBase.poolEndDate &&
        (this.dataBase.poolEndDate =
          select && select.ddpValidUntil
            ...
            : "");
      !this.dataBase.contractDdpStatus &&
        (this.dataBase.contractDdpStatus = this.isadopt(
          this.dataBase.poolEndDate,
          2
        ));
  ...
  public isForeign(data) {
    if (this.dataBase.isPrebookApply == "1") {
      ...
      this.foreignTradeOffPrebook =
        foreignTradeCompany == foreignTradeCompanyprebook ? false : true;
    }
  }
  ...
  public changeEntryMode(value: any) {
    ...
    if (this.dataBase.entryMode === "STOCK") {
      ...
      this.conTable &&
        this.validateForm
          .get("actualSales")!
          .setValidators([Validators.required, this.cheakMail]); // 实际销售人
      this.validateForm
  ...
  ngOnChanges(changes: SimpleChange) {
    ...
    if (this.dataBase) {
      ...
      if (this.dataBase.hospitalNature) {
        ...
        const status = this.dataBase.detail.status;
        if (
          (flag == "0" && status == "DHTGYBTX") ||
          (flag == "0" && status == "XJDHTGYBTX")
        ) {
          this.prebookOff = false;
        }
        const ASYNS = async () => {
          let getPool: any = await this.getPoolList();
          const contractBuyer2 = getPool.find(
        ...
        if (status === "DHTOASH" && flag == "0") {
          //如果是oa审核节点放开备注的禁用
          this.dataBase.shipmentDelivery == "1" &&
            this.validateForm.controls.shipmentDelivery.enable();
          this.dataBase.performanceBond == "1" &&
            this.validateForm.controls.performanceBond.enable();
          this.dataBase.installationWarranty == "1" &&
            this.validateForm.controls.installationWarranty.enable();
          this.dataBase.sitePreparation == "1" &&
            this.validateForm.controls.sitePreparation.enable();
          this.dataBase.afterSales == "1" &&
            this.validateForm.controls.afterSales.enable();
          this.supportFileMissingFlag();
          this.dataBase.amountDifference == "1" &&
            this.validateForm.controls.amountDifference.enable();
      ...
      this.entryMode = this.dataBase.entryMode;
      if (
        this.dataBase.entryMode == "" ||
        this.dataBase.entryMode == "BIDDING" ||
        (this.dataBase.sampleAuditFlag == "1" &&
          this.dataBase.entryMode == "STOCK")
      ) {
        this.rowspanht = 7;
      } else {
    ...
    if (this.dataBase && this.dataBase.entryMode) {
      if (this.dataBase.entryMode === "STOCK") {
        ...
        this.validateForm.get("endUserEmail")!.setValidators([this.cheakMail]);
        this.conTable &&
          this.validateForm.get("actualSales")!.clearValidators(); // 实际销售人
      } else {
        this.conTable &&
          this.validateForm
            .get("actualSales")!
    ...
    this.viewData(
      "bidWinningNotice",
      "bidWinningNoticeFileList",
      this.dataBase.bidWinningNoticeNames
    );
    this.viewData(
      "siteReport",
      "siteReportFileList",
      this.dataBase.siteReportNames
    );
    this.viewData(
      "projectSolutions",
      "projectSolutionsFileList",
      this.dataBase.projectSolutionsNames
    );
    this.viewData(
      "biddingDocuments",
      "biddingDocumentsFileList",
      this.dataBase.biddingDocumentsNames
    );
    this.viewData(
      "endUserContract",
      "endUserContractFileList",
      this.dataBase.endUserContractNames
    );
    this.viewData(
      "paymentProvisionFileName",
      "paymentProvisionFileNameList",
      this.dataBase.paymentProvisionFileNames
    );
    this.viewData(
      "shipmentDeliveryFileName",
      "shipmentDeliveryList",
      this.dataBase.shipmentDeliveryFileNames
    );
    this.viewData(
      "installationWarrantyFileName",
      "installationWarrantyList",
      this.dataBase.installationWarrantyFileNames
    );
    this.viewData(
      "amountDifferenceFileName",
      "amountDifferenceList",
      this.dataBase.amountDifferenceFileNames
    );
    this.viewData(
      "sitePreparationFileName",
      "sitePreparatList",
      this.dataBase.sitePreparationFileNames
    );

    this.viewData(
      "performanceBondFileName",
      "performanceBondList",
      this.dataBase.performanceBondFileNames
    );

    this.viewData(
      "afterSalesFileName",
      "afterSalesList",
      this.dataBase.afterSalesFileNames
    );
    this.viewData(
      "supportFileMissingFileName",
      "supportFileMissingList",
      this.dataBase.supportFileMissingFileNames
    );
    this.viewData(
      "otherFilName",
      "otherFilNameList",
      this.dataBase.otherFilNames
    );
    this.viewData(
      "projectAnalysisTable",
      "projectAnalysisTableFileList",
      this.dataBase.projectAnalysisTableNames
    );
    this.viewData(
      "tenderDocuments",
      "tenderDocumentsList",
      this.dataBase.tenderDocumentsNames
    );
    this.viewData(
      "confirmationFile",
      "confirmationFileFileList",
      this.dataBase.confirmationFileNames
    );
    this.viewData(
      "mrShieldingCompany",
      "mrShieldingCompanyList",
      this.dataBase.mrShieldingCompanyNames
    );
    this.setColSpanOfConfirmTable();
  }
  //招标编号
  ...
  setType() {
    if (
      this.dataBase.entryMode == "STOCK" &&
      (this.dataBase.userTeme == "BV" || this.dataBase.userTeme == "DXR")
    ) {
  ...
  selectModelFile() {
    if (
      ...
      this.dataBase.detail.status == "DHTGYBTX" ||
      this.dataBase.detail.status == "XJDHTGYBTX" ||
      this.dataBase.detail.status == "DBCWJSC"
    ) {
      this.rowspans = 2;
  ...
  setBaseInfor() {
    ...
    if (
      (this.dataBase.detail.status &&
        this.dataBase.detail.status !== "DOACS") ||
      this.showChek
    ) {
  ...
  public changeContract(event) {
    const select = this.contractCancelList.find(
      (val) => val.contractCancelReferenceId == event
    );
    if (select) {
  ...
  public ngOnInit(): void {
    ...
    this.validateForm = this.fb.group({
      ...
      addresseeTel: new FormControl({ value: "Nancy", disabled: this.disa }, [
        this.checkPhone,
      ]), //收件人电话
      addressee: new FormControl({ value: "Nancy", disabled: this.disa }), //收件人
      telTax: new FormControl({ value: "Nancy", disabled: this.disa }, [
        this.checkPhone,
      ]), //tel/tax
      accountAddress: new FormControl({ value: "Nancy", disabled: this.disa }), //开户地址
      taxNumber: new FormControl({ value: "Nancy", disabled: this.disa }, [
        Validators.required,
        this.taxNumberCheck,
      ]), //税号
      ...
      endUserAddress: new FormControl(
        { value: "Nancy", disabled: this.disa },
        Validators.required
      ),
      ...
      paymentProvision: new FormControl(
        { value: "Nancy", disabled: this.disa },
        Validators.required
      ), //合同概要表和进单装备表的基础验证差别
      shipmentDelivery: new FormControl(
        { value: "Nancy", disabled: this.disa },
        Validators.required
      ),
      installationWarranty: new FormControl(
        { value: "Nancy", disabled: this.disa },
        Validators.required
      ),
      // installationWarrantyRadio: new FormControl({value: 'Nancy', disabled: this.disa}, null), // 下一级是否审核
      amountDifference: new FormControl(
        { value: "Nancy", disabled: this.disa },
        Validators.required
      ),
      ...
      sitePreparation: new FormControl(
        { value: "Nancy", disabled: this.disa },
        Validators.required
      ),
      performanceBond: new FormControl(
        { value: "Nancy", disabled: this.disa },
        Validators.required
      ),
      supportFileMissing: new FormControl({
        value: "Nancy",
        disabled: this.disa,
      }),
      supportFileMissingRemarks: new FormControl({
        value: "Nancy",
        disabled: this.disa,
      }),
      ...
      paymentProvisionRemarks: new FormControl(
        { value: "Nancy", disabled: this.disa },
        Validators.required
      ),
      shipmentDeliveryRemarks: new FormControl(
        { value: "", disabled: this.disa },
        Validators.required
      ),
      installationWarrantyRemarks: new FormControl(
        { value: "Nancy", disabled: this.disa },
        Validators.required
      ),
      amountDifferenceRemarks: new FormControl(
        { value: "Nancy", disabled: this.disa },
        Validators.required
      ),
      sitePreparationRemarks: new FormControl(
        { value: "Nancy", disabled: this.disa },
        Validators.required
      ),
      performanceBondRemarks: new FormControl(
        { value: "Nancy", disabled: this.disa },
        Validators.required
      ),
      ...
      nmpaName: new FormControl(
        { value: "Nancy", disabled: true },
        Validators.required
      ),
      installationWarrantyRadio: new FormControl(
        {
          value: "Nancy",
          disabled:
            this.disa ||
      ...
      marketBundleQuantity: new FormControl(
        { value: "Nancy", disabled: true },
        null
      ),
      sofonNo: new FormControl(
        { value: "Nancy", disabled: true },
        Validators.required
      ),
  ...
  public handleOkCPResult2() {
    ...
    console.log("this.ckdealformlist", this.ckdealformlist);
    this.validateForm.reset();
    this.clearFrom();
    ...
    if (this.ckdealformlist) {
      setTimeout(() => {
        ...
        this.ServesiceService.sofonNosend.emit(
          this.ckdealformlist.finaSofonQuoation
        );
  ...
  ifBusinessModel() {
    ...
    if (this.dataBase && this.dataBase.businessModel === "DIRECT") {
      ...
      this.conTable &&
        this.validateForm
          .get("agreementNo")!
          .setValidators(Validators.required); //经销商协议号
  ...
  ifForeignTradeCompany() {
    ...
    if (this.dataBase && this.dataBase.invoiceInformation === "CNY") {
      ...
      this.validateForm
        .get("taxNumber")!
        .setValidators([Validators.required, this.taxNumberCheck]);
      this.validateForm
        .get("accountAddress")!
        .setValidators(Validators.required);
  ...
  GetDealLists(param) {
    return new Promise((resolve, reject) => {
      this.http
        .get(`/act/preparation/queryCp?dealFormId=` + param)
        .subscribe((e) => {
          resolve(e.data[0]);
  ...
  GetDealList() {
    ...
    this.box = true;
    this.http
      .get(`/act/preparation/queryCp?dealFormId=` + this.dealFormIdinput)
      .subscribe(
        (e) => {
    ...
    if (item.taskStatus && item.taskStatus === "YZBQRDBCWJ") {
      window.open(
        location.origin +
          environment.base_href +
          "/#/" +
          "bidding/support-up?id=" +
          codeString(item.zbMainId) +
          "&flag=1" +
          "&status=" +
          item.taskStatus
      );
  ...
  public showDiag() {
    ...
    if (!this.disa) {
      const select = this.dealList.find(
        (val) => dealerAgreementNo === val.agreementNo
      );
      if (select) {
        const obj = {
      ...
      const obj = {
        ...
        authorizedProduct:
          this.dataBase.preparationProductCompany.authorizedProduct,
      };
      this.dealshow.data.push(obj);
      this.ServesiceService.dealTable.emit(this.dealshow);
    }
  ...
  public supportFileMissingFlag() {
    ...
    if (state === "DHTOASH") {
      if (
        (this.dataBase.biddingDocuments == "" ||
          this.dataBase.biddingDocuments == null ||
          this.dataBase.biddingDocuments == undefined) &&
        this.dataBase.tenderNo != "其他类型"
      ) {
        flag = true;
      }
      if (
        (this.dataBase.tenderDocuments == "" ||
          this.dataBase.tenderDocuments == null ||
          this.dataBase.tenderDocuments == undefined) &&
        this.dataBase.tenderNo != "其他类型"
      ) {
        flag = true;
      }
      if (
        (this.dataBase.endUserContract == "" ||
          this.dataBase.endUserContract == null ||
          this.dataBase.endUserContract == undefined) &&
        this.dataBase.businessModel != "DIRECT"
      ) {
        flag = true;
      }
      if (
        (this.dataBase.projectAnalysisTable == "" ||
          this.dataBase.projectAnalysisTable == null ||
          this.dataBase.projectAnalysisTable == undefined) &&
        this.dataBase.businessModel == "DISTRIBUTOR"
      ) {
        flag = true;
      }
      this.dataBase.supportFileMissing == "1" &&
        !flag &&
        this.validateForm.controls.supportFileMissing.enable();
    }
  }
  public upmode = true;
  ...
  public getfilelist() {
    ...
    if (dealFormId != "" && dealFormId != undefined && dealFormId != null) {
      this.http
        .get(
          `/act/preparation/getAttachmentFromCP/` +
            dealFormId +
            this.sofonFile_ClassType
        )
        .subscribe(
          (res) => {
  ...
  public handleOkSofon(): void {
    ...
    this.dataBase.sofonNameurl =
      this.tranfSingle.checkOptionsOne[0].sofonFileUrl;
  ...
  handleOk(): void {
    ...
    if (this.fileChecked != null && this.fileChecked.length == 1) {
      this.http.get("/act/system/upload/cp/" + this.fileChecked[0]).subscribe(
        (res1) => {
          if (res1.code == "0000") {
      ...
      // ======================================  上传sofon文件
      this.http.post("/act/system/upload/cp", this.fileChecked).subscribe(
        (res1) => {
          if (res1.code == "0000") {
            // this.uploadZipFileName = res1.data.FileName;
            this.dataBase.sofonFileName = res1.data.FileName;
  ...
  public updateDisaSofonFile() {
    ...
    this.disa_sofon_file =
      this.dataBase.detail.flag !== "0" ||
      this.dataBase.detail.status !== "DHTOASH";
  ...
  handleImportByDealFormId(dealFormId) {
    ...
    this.http
      .get(`/act/preparation/queryCp?dealFormId=` + dealFormId)
      .subscribe(
        (e) => {
          if (e.data) {
            this.dealformlist = e.data;
