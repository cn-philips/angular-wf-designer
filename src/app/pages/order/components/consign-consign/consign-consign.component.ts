...
import { decodeString, formatDatesNow, formatDates, getType, upLoadFiles, standardTime, isadopt } from '@core/util/tools';
...
export class ConsignConsignComponent implements OnInit {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.http.post(`/act/process/getProcessWorkHisInfo`, params).subscribe(rest => {
        if (rest.code === '0000') {
          ...
          if (roleOff) {
            this.user = JSON.parse(localStorage.getItem("roleAgents"));
            let owner = listOfData.filter(vals => this.user.indexOf(vals.assignee) > -1);
            this.disas = (owner && owner.length > 0 && this.state != 'change_oit_approval' && this.state != 'change_oit') ? false : true;
  ...
  ngOnChanges() {
    ...
    if (this.mergeData.hospitalNature) {

      const ASYNS = async () => {
        if (this.mergeData.businessModel == 'DISTRIBUTOR') {
  ...
  getdistributorDate() {
    let param = {
      ...
      agreementNo: "", //协议号 
      dealerCode: "", //经销code
      dealerName: this.mergeData.agent, //经销商名称
      selectName: "", //当前选中
    }
  ...
  getIepoolDate() {
    let param = {
      corporateName: this.mergeData.foreignTradeCompany,
    }
