...
import { codeString, getType } from '@core/util/tools';
class QueryParams {
  loading: boolean;
  total: number;
  pageNo: number;
...
export class BusinessConfigComponent implements OnInit {
  ...
  editScene(item) {
    ...
    (item.firstApproved&&item.firstApproved.constructor === String) && (item.firstApproved = item.firstApproved.split(","));
    (item.secondApproved&&item.secondApproved.constructor === String) && (item.secondApproved = item.secondApproved.split(","));
  ...
  clickSwitch = (item) => {
    const obj = item;
    obj.status = obj.status == true ? 1 : 0;
    const { id, status, orderChange, describes, approver,firstApproved,secondApproved,changeDealForm } = obj;
    const saveData = { id, status, orderChange, describes, approver,firstApproved,secondApproved,changeDealForm };
    this.http.post(`act/ecom/order/application/saveAndSubmit`, saveData).subscribe((rest => {
      if (rest.code === '0000') {
        this.message.create('success', rest.msg);
        //this.refreshTable();
  ...
  public changePageSizeOnwer(e) {
    this.paramsOnwer.pageNo = 1
    this.paramsOnwer.pageSize = e;
    this.getTabListShortData()
  }
  // 查看跳转
  public toEntrust(id) {
    this.router.navigate(['/system-setting/change-owner'], {
