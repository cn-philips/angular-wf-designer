...
@Component({
  selector: "cos-waite-me-supplement-search-item",
  templateUrl: "search-item.component.html",
  styleUrls: ["search-item.component.scss"],
})
export class WaiteMeSupplementSearchItemComponent implements OnInit {
  ...
  public setTabChange() {
    let markLabel = "WaitingMyHandleState";
    let existState = localStorage.getItem(markLabel);
    if (existState){
      let existStateObj = JSON.parse(existState);
      let isHandle = existStateObj[this.activeRoute.url["_value"][0].path]
      if (isHandle !== null && isHandle !== "" && isHandle !== undefined) {
        this.isHandle = isHandle;
      }
      existStateObj[this.activeRoute.url["_value"][0].path] = 0
      localStorage.setItem(markLabel, JSON.stringify(existStateObj));
      this.submitForm(null, this.formValues.value);
    }

    this.routerEventsListener = this.router.events
      .filter((event) => event instanceof NavigationStart)
      .subscribe((event: any) => {
        let state = localStorage.getItem(markLabel);
        if (!state) {
          localStorage.setItem(markLabel, JSON.stringify({}));
        }
        let stateObj = JSON.parse(localStorage.getItem(markLabel));
        stateObj[this.activeRoute.url["_value"][0].path] = this.isHandle;
        localStorage.setItem(markLabel, JSON.stringify(stateObj));
      });
  }
  ngOnDestroy() {
  ...
  submitForm($event: any, value: any) {
    ...
    //分隔调用
    setTimeout(()=>{this.search.emit(value)},0);
  }
  ...
  resetForm(value: any) {
    ...
    this.loading = true;
    this.setLoading.emit(this.loading);
    this.search.emit({ isHandle: this.isHandle });
  }
