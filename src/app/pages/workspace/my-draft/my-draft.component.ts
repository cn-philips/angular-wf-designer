...
import { DraftComponent } from "../../special-approval/special-approval.module";

@Component({
  selector: "cos-my-draft",
...
export class MyDraftComponent implements OnInit {
  ...
  @ViewChild("spTable") spTable: DraftComponent;
  ...
  getTableData() {
    ...
    this.http.post(`/act/ecos/apply/findDraft`, params).subscribe(
      (rest) => {
        if (rest.code === "0000") {
  ...
  exportData() {
    ...
    this.http.postDownload(`/act/ecos/report/export/findDraft`, params).subscribe(rest => {
