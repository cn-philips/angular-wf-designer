...
export class RequestFormComponent implements OnInit {
  ...
  @ViewChild("rddOitOrderInfo")
  public rddOitOrderInfo: RddOitOrderInfoComponent;
  ...
  public formValues = this.fb.group({
    ...
    changeOrderInfos: this.fb.group({
      ...
      orders: this.fb.array([
        this.fb.group({
          ...
          approvalConfigSecond: [{ value: null, disabled: true }], // 销售区域配置
        ...
        this.fb.group({
          ...
          exchangeRole: [null, [Validators.required]], // 换货角色
          saleEmail: [null, [Validators.required]], // 销售邮箱
          approvalConfigSecond: [null, [Validators.required]], // 销售邮箱
          districtLeader: [{ value: null, disabled: true }], // District Leader邮箱
          salesLeader: [{ value: null, disabled: true }], // sales Leader 邮箱
          expectedSaleDate: [null, [Validators.required]], // 预计记认销售日期
          products: [[], [Validators.required]],
    ...
    transferLibOrders: this.fb.group({
      orders: this.fb.array([
        // 转库订单详情
        this.fb.group({
          ...this.transferLibOrderInit,
          applyId: null,
          id: null,
          approvalConfigSecond: [null, [Validators.required]],
        }),
        this.fb.group({
          ...this.transferLibOrderInit,
          applyId: null,
    ...
    deBookOrderInfos: [[]],
    lastBuyInfos: [[]],
    cancelorderInfo: this.fb.group({
      ...this.cancelOrderInit,
      applyId: null,
      id: null,
      isDeleted: 0,
    }),
    orderReplacementInfo: this.fb.group({
      ...this.orderReplacementInit,
      applyId: null,
      id: null,
    }),
  ...
  parseRegion(region) {
    let arr = [
      region.funcTeamType == "0" || !!!region.funcTeamType
        ? region.team
        : region.serveTeam,
      region.modality,
      region.cycleGroup,
  ...
  initSaleRegions(role, isNewRequest = false) {
    const regions = (JSON.parse(window.localStorage.getItem("profiles")) || [])
      ...
      .map((region) => ({
        ...region,
        label: this.parseRegion(region),
        value: this.parseRegion(region),
      }));
    ...
    if (isNewRequest && regions.length === 1) {
      ...
      this.basicInfo.patchValue({
        systemRegion: [
          funcTeamType == "0" || !!!funcTeamType ? team : serveTeam,
          modality,
          cycleGroup,
          bigArea,
          smallArea,
    ...
    if (
      ...
      this.orderInfo.patchValue({
        products: [
          {
            warranty: {
              stdWarrantyMonths: this.spService.standWarrantyMonth["PD&IGT"],
            },
          },
        ],
      });
  ...
  public ngOnInit(): void {
    ...
    if (requestId) {
      ...
      this.applicantEmail = localStorage.getItem("ecom_ng_philips_code1");
      ...
      if (bg) {
        switch (type) {
          ...
          case APPLY_TYPE.TRANSFER_LIB: // 给转库添加默认BG
            let transferOrder = this.transferLibInfos.get(
              "orders"
            ) as FormArray;
  ...
  public setFormValidators(type, item, bg) {
    ...
    switch (type) {
      case APPLY_TYPE.EXT_WARRANTY:
        ...
        clearedFields.forEach((fieldName) =>
          this.orderInfo.controls[fieldName].clearValidators()
        );
        if (item === "sp_warranty_apply_item_5") {
          this.basicInfo.controls.applyItemDesc.setValidators([
            Validators.required,
          ]);
        }
        break;
      ...
      case APPLY_TYPE.PRE_BOOK_LASTBUY:
        this.basicInfo.controls.lastBuyPlan.setValidators([
          Validators.required,
        ]);
        break;
      case APPLY_TYPE.CANCEL_ORDER:
        if (bg == "CC") {
          ...
          clearedFields.forEach((fieldName) =>
            this.cancelorderInfo.controls[fieldName].clearValidators()
          );
        } else {
          this.cancelorderInfo.controls.productType.setValidators([
            Validators.required,
          ]);
        }
        if (bg == "PD&IGT") {
    ...
    if (bg === "PD&IGT") {
      ...
      if (
        type !== APPLY_TYPE.LC_AMENDMENT &&
        type !== APPLY_TYPE.TRANSFER_LIB
      ) {
  ...
  public getFormData() {
    ...
    switch (this.applyType) {
        ...
        data.orderInfos = [
          {
            ...
            ...orderInfo,
            applyArrivalTime: applyArrivalTime
              ? moment(applyArrivalTime).format("YYYY-MM-DD")
              : null,
            expectedPaymentDate: expectedPaymentDate
              ? moment(expectedPaymentDate).format("YYYY-MM-DD")
              : null,
            expectedSitePlaceDate: expectedSitePlaceDate
              ? moment(expectedSitePlaceDate).format("YYYY-MM-DD")
              : null,
            expectedSaleDate: expectedSaleDate
              ? moment(expectedSaleDate).format("YYYY-MM-DD")
              : null,
            products: products.map(
              ({ productType, wbsNo, itemNo, quantity }) => ({
                productType,
                wbsNo,
                itemNo,
                quantity,
        ...
        data.orderInfos = [
          {
            ...
            products: products.map((product) => ({
              ...
              warranty: {
                ...product.warranty,
                applyStdWarrantyEnddate: moment(
                  product.warranty.applyStdWarrantyEnddate
                ).format("YYYY-MM-DD"),
                expectedStdWarrantyStartdate: moment(
                  product.warranty.expectedStdWarrantyStartdate
                ).format("YYYY-MM-DD"),
                applyExtWarrantyMonths: Number(
                  product.warranty.applyExtWarrantyMonths
                ),
              },
            })),
          },
      ...
      case APPLY_TYPE.LC_AMENDMENT: // LC_AMENDMENT申请
        ...
        const {
          lcInfo: { modifyEntry, cancelReason },
        } = lcAmendmentOrderInfo;
        data.orderInfos = [
          {
            ...originOrderInfo,
            ...lcAmendmentOrderInfo,
            productType: Array.isArray(lcAmendmentOrderInfo.productType)
              ? lcAmendmentOrderInfo.productType.join(",")
              : lcAmendmentOrderInfo.productType,
            lcInfo: {
              ...originOrderInfo.lcInfo,
              ...lcAmendmentOrderInfo.lcInfo,
      ...
      case APPLY_TYPE.LOGISTICSCOST:
        ...
        break;
      case APPLY_TYPE.MACHINE_EXCHANGE:
        const config1 = changeOrderInfos.orders.at(1).approvalConfigSecond;
        data.orderInfos = [
          {
            transferCargo: "sp_machineexchange_order_type_item_1",
            ...this.requestInfo.orderInfos[0],
            ...changeOrderInfos.orders.at(0),
            expectedSaleDate: changeOrderInfos.orders.at(0).expectedSaleDate
              ? moment(changeOrderInfos.orders.at(0).expectedSaleDate).format(
                  "YYYY-MM-DD"
                )
              : null,
            products: changeOrderInfos.orders
              .at(0)
              .products.map(
                ({
                  productType,
      ...
      case APPLY_TYPE.TRANSFER_LIB: // Additional cost
        const transLibConfig0 =
          transferLibOrders.orders.at(0).approvalConfigSecond;
        const transLibConfig =
          transferLibOrders.orders.at(1).approvalConfigSecond;
        ...
        this.requestInfo.orderInfos.forEach((order) => {
          if (order.transferCargo === "sp_transferlib_order_type_item_1") {
            data.orderInfos[0] = {
              ...order,
              ...data.orderInfos[0],
              approvalConfigSecond: data.orderInfos[0].approvalConfigSecond,
            };
          } else if (
            order.transferCargo === "sp_transferlib_order_type_item_2"
          ) {
            data.orderInfos[1] = {
              ...order,
              ...data.orderInfos[1],
              approvalConfigSecond: data.orderInfos[1].approvalConfigSecond,
            };
          }
        });
        data.extInfo = {
      ...
      case APPLY_TYPE.RDD_OIT:
        ...
        rddOitOrderInfos.forEach((rddOitOrderInfo) => {
          const {
            ...
            // productInfo
            deliveryDelayReason,
            exchangeableHospitalName,
            exchangeableHospitalNo,
            exchangeableOrder,
            exchangeableOrderModel,
            exchangeableOrderSale,
            exchangeableOrderSaleBigArea,
            exchangeableOrderSaleCycleGroup,
            exchangeableOrderSaleDate,
            exchangeableSoNo,
            exchangeableWbsNo,
            newRdd,
            originalRdd,
          ...
          const product = {
            ...
            exchangeableOrderSaleBigArea,
            exchangeableOrderSaleCycleGroup,
            exchangeableOrderSaleDate: exchangeableOrderSaleDate
              ? moment(exchangeableOrderSaleDate).format("YYYY-MM-DD")
              : null,
            originalRdd: originalRdd
              ? moment(originalRdd).format("YYYY-MM-DD")
              : null,
      ...
      case APPLY_TYPE.CANCEL_ORDER: //cancel order
        data.orderInfos = [
          {
            ...cancelorderInfo,
            productType: Array.isArray(cancelorderInfo.productType)
              ? cancelorderInfo.productType.join(",")
              : cancelorderInfo.productType,
            orderInfoStatus: {
        ...
        data.orderInfos = [
          {
            ...this.requestInfo.orderInfos[0],
            ...noneDirectOrderInfo,
            expectedSaleDate: noneDirectOrderInfo.expectedSaleDate
              ? moment(noneDirectOrderInfo.expectedSaleDate).format(
                  "YYYY-MM-DD"
                )
              : null,
            expectedSignAcDate: noneDirectOrderInfo.expectedSignAcDate
              ? moment(noneDirectOrderInfo.expectedSignAcDate).format(
                  "YYYY-MM-DD"
                )
              : null,
            products: noneDirectOrderInfo.products.map(
              ({ productType, wbsNo, itemNo, equipmentSn, quantity }) => ({
                productType,
                wbsNo,
                itemNo,
                equipmentSn,
      ...
      case APPLY_TYPE.DE_BOOK:
        ...
        deBookOrderInfos.forEach((debookOrderInfo) => {
          ...
          if (!saps.includes(sapOrderNo)) {
            ...
            const debookorder = data.orderInfos.filter(
              (value) => value.sapOrderNo === sapOrderNo
            );
      ...
      case APPLY_TYPE.PRE_BOOK_LASTBUY:
        ...
        lastBuyInfos.forEach((lastBuyInfo) => {
          ...
          if (!lastBuySaps.includes(sapOrderNo)) {
            ...
            const lastbuyorder = data.orderInfos.filter(
              (value) => value.sapOrderNo === sapOrderNo
            );
  ...
  setLcAmendmentOrderInfoFormValidators(orderInfo) {
    ...
    const {
      lcInfo: { acceptLcDiscrepancy, modifyEntry, cancelReason },
    } = orderInfo;
    switch (applyItem) {
      ...
      case "sp_lcamendment_apply_item_2":
        ...
        if (acceptLcDiscrepancy === 1) {
          lcInfoControls.lcDiscrepancyPaymentMethod.setValidators([
            Validators.required,
          ]);
        }
        break;
  ...
  public async onSubmit() {
    ...
    switch (this.applyType) {
      ...
      case APPLY_TYPE.MACHINE_EXCHANGE:
        ...
        if (
          product0.value[0].logisticsStatus !== 1 &&
          product1.value[0].logisticsStatus !== 1
        ) {
          this.message.error("请至少提交一条已到货产品");
          return;
        }
        if (
          (product0.value[0].logisticsStatus === 0 ||
            product0.value[0].logisticsStatus === 1) &&
          !product0.value[0].equipmentSn
        ) {
          this.message.error("请按要求填写设备SN");
          return;
        }
        if (
          (product1.value[0].logisticsStatus === 0 ||
            product1.value[0].logisticsStatus === 1) &&
          !product1.value[0].equipmentSn
        ) {
          this.message.error("请按要求填写设备SN");
          return;
        }
      ...
      case APPLY_TYPE.TRANSFER_LIB:
        const transferLibOrder = this.transferLibInfos.get(
          "orders"
        ) as FormArray;
        let formValidError = false;
        if (
          transferLibOrder.at(0).get("bmc").value !==
          transferLibOrder.at(1).get("bmc").value
        ) {
          this.message.error("转入转出bmc不一致，请重新选择");
          return;
        }
        if (
          (transferLibOrder.at(0).get("referenceId").value && transferLibOrder.at(1).get("referenceId").value)
          && (transferLibOrder.at(0).get("referenceId").value == transferLibOrder.at(1).get("referenceId").value)
        ) {
          this.message.error("转入转出ReferenceId不能相同，请重新选择");
          return;
        }
        if (
          !transferLibOrder.at(0).get("hospitalName").value ||
          !transferLibOrder.at(1).get("hospitalName").value
        ) {
          this.message.error("请选择医院再提交");
          return;
        }
        ...
        const difference =
          this.orderDifferencesInfo.get("orderDifferences").value;
        if (!difference || difference.length === 0) {
          ...
          for (let i = 0; i < difference.length; i++) {
            if (
              !difference[i].configDetail ||
              !difference[i].transferOut ||
              !difference[i].transferIn ||
              !difference[i].handlePlan ||
              difference[i].cost == null
            ) {
      ...
      case APPLY_TYPE.CANCEL_ORDER:
        ...
        const orderInfoStatus = this.cancelorderInfo.get(
          "orderInfoStatus"
        ) as FormGroup;
    ...
    if (this.applyType === APPLY_TYPE.EXT_WARRANTY) {
      if (
        ...
        orderInfos[0].products.length === 0
      ) {
        ...
        let hasError = false;
        let errorMsg = "";
        orderInfos[0].products.forEach(
          ({ warranty: { applyExtWarrantyMonths } }) => {
  ...
  public async onSaveDraft() {
    const id = this.message.loading(LOADING_MESSAGE.SAVE_DRAFT, {
      nzDuration: 0,
    }).messageId;
  ...
  public async onApproveSave() {
    ...
    try {
      ...
      const { remark, attachments, notify, notifier } =
        this.supplementFormValues.getRawValue();
  ...
  public async onFeedBackSubmit(action: number) {
    ...
    if (!hasError) {
      try {
        const { remark, attachments, notify, notifier } =
          this.feedBackFormValues.getRawValue();
        const formData = this.getFormData();
        const data = {
          applyId: this.requestId,
          attachments: attachments,
      ...
      switch (this.applyType) {
        ...
        case APPLY_TYPE.PRE_BOOK_LASTBUY: //cancel order 补充信息必填
          for (let i = 0; i < this.lastBuyInfos.value.length; i++) {
            if (
              !this.lastBuyInfos.value[i].actualOitDate ||
              !this.lastBuyInfos.value[i].warehouseArrangement
            ) {
              hasError = true;
              return;
            }
    ...
    try {
      ...
      this.formValues.patchValue({
        basicInfo: {
          ...
          systemRegion:
            bg && cycleGroup
              ? [team, bg, cycleGroup, bigArea, smallArea]
                  .filter((str) => str && str.trim())
                  .join("-")
              : null,
          bg,
      ...
      if (
        applyType === APPLY_TYPE.PRODUCTION ||
        applyType === APPLY_TYPE.EXT_WARRANTY ||
        applyType === APPLY_TYPE.LOGISTICSCOST ||
        applyType === APPLY_TYPE.EXT_INSTALL_COST ||
        applyType === APPLY_TYPE.SPECIAL_DELIVERY
      ) {
        ...
        const {
          lcInfo: { cancelReason, modifyEntry },
        } = orderInfo;
        ...
        if (
          ...
          cancelReasons.includes("sp_lc_other_cancel")
        ) {
          this.lcAmendmentOrderInfo
            .get("lcInfo")
            .get("cancelReasonDesc")
            .setValidators(Validators.required);
        }
        if (
          Array.isArray(modifyEntries) &&
          modifyEntries.includes("sp_lc_other_modify")
        ) {
          this.lcAmendmentOrderInfo
            .get("lcInfo")
            .get("modifyEntryDesc")
        ...
        this.formValues.patchValue({
          lcAmendmentOrderInfo: {
            ...
            productType:
              orderInfo.bg === "US" && orderInfo.productType
                ? orderInfo.productType.split(",")
        ...
        let orders1 = null;
        if (
          orderInfos[0].transferCargo === "sp_machineexchange_order_type_item_1"
        ) {
        ...
        let order1 = null;
        if (
          orderInfos[0].transferCargo === "sp_transferlib_order_type_item_1"
        ) {
          order0 = orderInfos[0];
          order1 = orderInfos[1];
        } else {
        ...
        this.formValues.patchValue({
          transferLibOrders: {
            orders: [
              ...
              },
              {
                ...
                approvalConfigSecond: order1.approvalConfigSecond,
              },
            ],
          },
          exchangeInfo: {
          ...
          basicInfo: {
            systemRegion: order1.approvalConfigSecond,
          },
        });
        this.setFormValidators(applyType, applyItem, orderInfos[0].bg);
        ...
        const attachmentList =
          orderInfos[0].orderInfoStatus.cancleOrderAttachment || [];
        this.cancelOrderFileList = attachmentList.map(
        ...
        this.formValues.patchValue({
          cancelorderInfo: {
            ...orderInfos[0],
            productType:
              orderInfos[0].bg === "US" && orderInfos[0].productType
                ? orderInfos[0].productType.split(",")
                : orderInfos[0].productType,
            orderInfoStatus: {
        ...
        orderInfos.map((order) => {
          order.products.map(
            ({ wbsNo, productType, deBookReason, remark, orderDate }) => {
              debooks.push({
                ...order,
                wbsNo: wbsNo,
        ...
        orderInfos.map((order) => {
          if (
            order.stockingAgreementFile &&
            order.stockingAgreementFile.length > 0
          ) {
            order.stockingAgreementFileList = order.stockingAgreementFile.map(
              ({ fileId, name, size, type }) => ({
                uid: fileId,
                fileId,
                name,
      ...
      nodeInfoList.forEach(({ approverList }) =>
      ...
      this.supportFileList = attachments.map(
        ({ fileId, name, size, type }) => ({
          uid: fileId,
          fileId,
          name,
      ...
      if (
        [PROCESS_STATUS.REJECTED, PROCESS_STATUS.WITHDRAW].includes(
          processStatus
        ) &&
        this.isApplicant &&
        status !== 0
      ...
      //     return approverList.find(({ user }) => user === localStorage.getItem('ecom_ng_philips_code1'));
      //   } else {
      //     return false;
      //   }
      // });
  ...
  public async onCancelRequest() {
    const id = this.message.loading(LOADING_MESSAGE.CANCEL_REQUEST, {
      nzDuration: 0,
    }).messageId;
    try {
      this.submitLoading = true;
  ...
  public async onWithdrawRequest() {
    const id = this.message.loading(LOADING_MESSAGE.WITHDRAW_REQUEST, {
      nzDuration: 0,
    }).messageId;
    try {
      this.submitLoading = true;
  ...
  public navigateToHomePage() {
    this.router.navigate(["/ecos/home"]);
    // this.routerExtend.back();
  }
    ...
    if (
      ...
      if (orderInfo && orderInfo.products && orderInfo.products.length > 0) {
        for (let i = 0; i < orderInfo.products.length; i++) {
          if (
            this.isEmpty(orderInfo.products[i].productType) ||
            this.isEmpty(orderInfo.products[i].wbsNo) ||
            this.isEmpty(orderInfo.products[i].itemNo) ||
            this.isEmpty(orderInfo.products[i].quantity)
          ) {
            this.message.error(errorMsg);
            return false;
          }
  ...
  public setMachineDefaultInfo() {
    if (this.editable) {
      ...
      orders.at(0).patchValue({
        saleEmail: localStorage.getItem("ecom_ng_philips_code1"),
      });
