...
export class ReferenceLinkComponent implements OnInit {
  ...
  public async gotoLink() {
    if (this.orderTypes && this.orderTypes !== '' && this.orderTypes.toUpperCase() === 'OIT') {
      // 跳转到合同概要表
      // window.open(location.origin + environment.base_href + '/#/' + 'pre-order/in-con-modif?id=' + codeString(this.cosMainId) + '&flag=1', null, 'noreferrer');
      window.open(location.origin + environment.base_href + '/#/' + 'pre-order/complete-oit?id=' + codeString(this.cosMainId) + '&flag=1' + '&status=OITEND', null, 'noreferrer');
    } else if (this.orderTypes && this.orderTypes !== '') {
      // 跳转到prebook链接
      window.open(location.origin + environment.base_href + '/#/' + 'pre-book/prebook-so?id=' + codeString(this.cosMainId) + '&flag=1&status=prebook_end', null, 'noreferrer');
    }
  }
