...
type CheckerResult = CheckResult | Hospital | Dealer
...
export class RddOitOrderInfoComponent implements OnInit {
  ...
  hospitalChecker = new Map<string, BehaviorSubject<CheckerResult>>()
  dealerChecker = new Map<string, BehaviorSubject<CheckerResult>>()
  ...
  searchHospital(subject: BehaviorSubject<CheckerResult>, customerName: string) {
      ...
      .subscribe(({ code, data }) => {
        if (code === '0000') {
          ...
          if (rows.length > 0) {
            ...
            subject.next({ customerName, no })
          } else {
            subject.next(CheckResult.Error)
          }
        }
      })
  }
  ...
  checkImportedHospital(order, isExchange = false) {
    ...
    const subject = this.hospitalChecker.get(customerName) || new BehaviorSubject<CheckerResult>(CheckResult.Loading)
    if (!this.hospitalChecker.get(customerName)) {
      this.hospitalChecker.set(customerName, subject)
      this.searchHospital(subject, customerName)
    }
    subject.subscribe(res => {
      if (res != CheckResult.Loading) {
        if (res === CheckResult.Error) {
          if (isExchange) {
          ...
          const { customerName, no } = res as Hospital
          if (isExchange) {
  ...
  searchDealer(subject: BehaviorSubject<CheckerResult>, dealerName: string) {
    this.http.post(`/act/preparation/getDealersOnlyWithRegFlag`, { dealerName })
      .subscribe(({ code, data }) => {
        if (code === '0000') {
          const { rows } = data
          if (rows.length > 0) {
            const { dealerCode } = rows[0]
            subject.next({ dealerName, dealerCode })
          } else {
            subject.next(CheckResult.Error)
          }
        }
  ...
  checkImportedDealer(order) {
    ...
    const { dealerName } = order

    const subject = this.dealerChecker.get(dealerName) || new BehaviorSubject<CheckerResult>(CheckResult.Loading)
    if (!this.dealerChecker.get(dealerName)) {
      this.dealerChecker.set(dealerName, subject)
      this.searchDealer(subject, dealerName)
    }
    ...
    subject.subscribe(res => {
      if (res != CheckResult.Loading) {
        if (res === CheckResult.Error) {
          ...
          const { dealerCode } = res as Dealer
