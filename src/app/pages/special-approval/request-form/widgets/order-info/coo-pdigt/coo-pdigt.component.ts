...
export class CooPdIgtOrderInfoComponent implements OnInit, OnChanges {
  ...
  loginUserCode1 = localStorage.getItem('ecom_ng_philips_code1')
  ...
  cooInfo = this.fb.group({
    ...
    applySignedDate: [calcApplySignedDate(), [Validators.required]], // 预计签署日期
  ...
  get omList() {
    return (this.spService.omUserMap['PD&IGT'] || []).map(({ name, email }) => ({
      label: `${name}(${email})`,
      value: email
    }))
  }
