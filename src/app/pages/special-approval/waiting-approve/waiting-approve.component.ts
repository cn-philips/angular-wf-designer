...
export class WaitingApproveComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.selectOptions.roleList = userRoleList.map((item) => ({
      label: item,
      value: item,
    }));
  }
  ...
  checkAll(value: boolean): void {
    this.tableData.list.forEach(
      (item) => (this.mapOfCheckedId[item.applyCode] = value)
    );
    this.refreshStatus();
  }

  refreshStatus(): void {
    this.allChecked = this.tableData.list.every(
      (item) => this.mapOfCheckedId[item.applyCode]
    );
    this.indeterminate =
      this.tableData.list.some((item) => this.mapOfCheckedId[item.applyCode]) &&
      !this.allChecked;
  }
  ...
  // 获取角色
  async onRolesChange() {
    const role = this.reassignParams.get("role").value
      ? this.reassignParams.get("role").value
      : "";
    if (role === "") {
    ...
    this.selectOptions.userList = userLists.map(({ email, name }) => ({
      label: `${name}(${email})`,
      value: email,
    }));
  }
  ...
  getFormData() {
    ...
    selectedList.forEach((value) => {
      let list = this.tableData.list
        .filter((item) => {
          return item.applyCode === value;
        })
        .map(({ applyId, taskInstId, procInstId }) => ({
          applyId,
          taskInstId,
          procInstId,
        }));
    ...
    //获取模态框数据
    const { role, receiver, transferReason } =
      this.reassignParams.getRawValue();
