...
import { HttpService, DictService } from '@core/services/index'
...
export class SpecialApprovalService {
  ...
  async initOmList() {
    const url = `/act/role/getUsersByRoleAndModality?role=OM`

    const pdIgtOM = this.http.get(url, { params: { modality: 'PD&IGT' } }).toPromise()
    const usOM = this.http.get(url, { params: { modality: 'US' } }).toPromise()
    const ccOM = this.http.get(url, { params: { modality: 'CC' } }).toPromise()

    Promise.all([pdIgtOM, usOM, ccOM]).then(([pdIgtOMRes, usOmRes, ccOmRes]) => {
      this.omUserMap['PD&IGT'] = pdIgtOMRes.data
  ...
  async getRoleBg() {
    const uri =
      "/act/role/getUsersByEmail?email=" + localStorage.ecom_ng_philips_code1;
    const res = await this.http.get(uri).toPromise();
    const data = formatResponse(res);
    return data;
  }
  ...
  //根据邮箱获取指定角色信息
  async getRolesInfo(email: string) {
    const uri =
      "/act/role/getDiigtUserInfoByEmail/" + email;
    const res = await this.http.get(uri).toPromise();
    const data = formatResponse(res);
    return data;
  }
