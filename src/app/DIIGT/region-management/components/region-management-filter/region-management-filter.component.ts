...
import {NzMessageService, NzTreeBase, NzTreeComponent, NzTreeNode} from 'ng-zorro-antd';
...
export class RegionManagementFilterComponent implements OnInit {
 ...
 saveArea(){
    ...
    if (this.regionNode.parentNode.origin.children.find(x => x.title == this.area.areaName)){
      console.log(this.regionNode.parentNode.origin.children.find(x => x.title == this.area.areaName));
