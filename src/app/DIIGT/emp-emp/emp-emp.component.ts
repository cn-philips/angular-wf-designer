...
export class EmpEmpComponent implements OnInit {
  ...
  row = [
    {
      name: 'Opportunity 1', Market: 'Market Bundle1', duc: [
        { name: '主产品1' },
        { name: '主产品2' }
      ], take: 'CP结果校验', end: 'CP结果已完成'
    },
    {
      name: 'Opportunity 1', Market: 'Market Bundle2', duc: [
        { name: '主产品1' }
      ], take: 'CP结果校验', end: 'CP结果已完成'
    },
    {
      name: 'Opportunity 2', Market: 'Market Bundle1', duc: [
        { name: '主产品1' }
      ], take: 'CP结果校验', end: 'CP结果已完成'
    }
  ];
  ...
  */
  ...
  public manufacturerAuthorizationLetter(code) {
    ...
    params.agentReceiver = localStorage.getItem('ng_philips_code1');
    ...
    params.date1 = today.getFullYear() + '-' + (today.getMonth() + 1) + '-' + today.getDate();
  ...
  CP(e) {
    ...
    if (this.data.productInformations.length > 0) {
      this.data.productInformations.map(res => {
        ...
        res.productInformations.map(vals => {
         ...
         // objs.marketBundleName=vals.productName;
          ...
          if (vals.productInformations && vals.productInformations.length > 0) // 是否有第三层
          {
      ...
      this.http.post(url, paramArr).subscribe(res=>{
        if (res.code === '0000') {
          this.load2 = false;
          this.bidData = [...arr];
          if (arrIscheak.length == 0) { // 是否有子产品
          ...
          this.bidData.map(item => {    // 添加dealFormId的标题
            let len = item.productList.length;
            item.rowspan = len > 0 ? len : 1;
            if (item.searchResult.length > 0) {
              item.searchResult.map(vals => {
                vals.temUser = false;
                vals.title = `dealFormId:${vals.dealFormId},makertBundleId:${vals.makertBundleId ? vals.makertBundleId : ''}`;
             });
            }
          });
        }
  ...
  getData() {
    const url = '/act/ecom/tender/application/getApprovalSubmit?mainId=' + this.mainid;
