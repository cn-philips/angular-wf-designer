...
import { decodeString, formatDatesNow, formatDatesNowMth, getType, upLoadFiles, viewDatas,codeString} from '../../../../assets/js/tools';
...
public getFormDetails(id) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.http.post(`/act/process/getProcessWorkHisInfo`, params).subscribe(rest => {
        if (rest.code === '0000') {          
          ...
          if (roleOff) {
            let user = localStorage.getItem("ng_philips_code1");
            let owner=listOfData.some(vals=>vals.assignee==user);  
            this.disas = (owner&&this.status!='change_oit_approval'&&this.status!='change_oit') ? false : true;
          }
          if (!this.disas) {
  ...
  updata() {
    ...
    // if(this.oitInfor.deBook=='0')
    // {
    //     this.message.create("error","De-book请选择是");
    //     return
    // }    
    if (this.oitInfor.deBook == '1') {
