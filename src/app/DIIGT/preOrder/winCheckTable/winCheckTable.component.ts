...
import { cheakbox } from '../servesice.service';
...
@Component({
  ...
  templateUrl: './winCheckTable.component.html',
  styleUrls: ['./winCheckTable.component.scss'],
  encapsulation: ViewEncapsulation.None,
  providers: [
  ],
})
export class PreOrderWinCheckTableComponent implements OnInit {
  radioValue = '';
  style = {
    display: 'block',
    height: '30px',
  ...
  selectClick(index, i) {  
    ...
    if (search.temUser) 
    {
      ...
      {        
        ...
        this.bidData[index].referenceId = this.bidData[index].searchResult[i].referenceId;
        this.bidData[index].productionInformId=this.bidData[index].searchResult[i].id;
  ...
  public gotoWin(item) {   
    console.log(location.origin + environment.base_href + '/#/' + 'winning?id=' + codeString(item.mainId) + '&flag=1');
    window.open(location.origin + environment.base_href + '/#/' + 'winning?id=' + codeString(item.mainId) + '&flag=1');
  }
