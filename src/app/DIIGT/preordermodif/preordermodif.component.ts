...
import { decodeString, formatDatesNow, standardTime } from '../../../assets/js/tools';
...
export class PreordermodifComponent implements OnInit {
      ...
      this.http.get(url).subscribe(res => {
        if (res.data) {
          ...
          if (productList.length > 0) {
            productList.map(res => {
              res.isPrebookApply = res.isPrebookApply != null ? res.isPrebookApply.toString() : "0";
              res.afterSales = (res.afterSales != null && res.afterSales != undefined && res.afterSales != "") ? res.afterSales : "0";
  ...
  saveContract(): void {
    ...
    this.dataBase.processInstanceTaskId = this.activatedRouter.queryParams['_value'].processInstanceTaskId;
    //清除第三层checked
    const productList = this.dataBase.productList;
    if (productList.length > 0) {
