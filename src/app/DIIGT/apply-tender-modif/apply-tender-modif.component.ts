...
export class ApplyTenderModifComponent implements OnInit {
  ...
  public save(): void {
    ...
    let productInformations = JSON.parse(JSON.stringify(this.product.productData));
  ...
  public submit(){
    ...
    //物流条款说明 非标准条款验证
    if (this.dataBase.logisticsTermsExplain != 'WLTKSMBZ' && this.dataBase.logisticsDescription === '收到信用证/货款90天内装运'){
    ...
    const prodcutLenth = this.dataBase.productInformations.every(res => res && res.productInformations && res.productInformations.length > 0); // 判断是否选择了产品
    if (!prodcutLenth) {
      this.myskip('complete-tab');
    ...
    const ASYNS = async () => {
      if (this.dataBase.businessType === 'DISTRIBUTOR') {
        if (this.dataBase.dealerNo === null || this.dataBase.dealerNo === 'null') {
          ...
          let i = await this.ddpJudge1(this.dataBase.dealerNo, this.dataBase.agreementAgenName);
          console.log(i);
          if (i === 1) {
            return;
          }
