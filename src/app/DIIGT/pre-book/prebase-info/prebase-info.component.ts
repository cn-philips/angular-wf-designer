...
import { getType, upLoadFile, checkPhone, decodeString, standardTime, formatDatesNow, formatDate, upLoadFileNew,codeString } from '../../../../assets/js/tools';
...
export class PrebaseInfoComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private message: NzMessageService,public activatedRouter: ActivatedRoute,private http: HttpService, private el: ElementRef, private ServesiceService: ServesiceService) {
  ...
  ngOnChanges(changes: SimpleChange) {
    ...
    this.ServesiceService.netPrice.subscribe(res => {
      ...
      if (this.dataBase.productList.length > 0) {
        ...
        this.dataBase.netPrice = this.returnFloat(Number(summay.toString().match(/^\d+(?:\.\d{0,4})?/)), 4);
      }
    });
    if (this.dataBase) {
    ...
    this.viewData("supportFile", "supportFileNameList", this.dataBase.supportFileName);
  ...
  ngOnInit() {
    ...
    this.validateForm = this.fb.group({
      ...
      endUserEmail: new FormControl({ value: 'Nancy', disabled: this.disa }, [this.cheakMail]),
      ...
      dealFormId: new FormControl({ value: 'Nancy', disabled: true }, Validators.required),//dealFormId
      dealContractPrice: new FormControl({ value: 'Nancy', disabled: true }, Validators.required),//dealForm总价
      netPrice: new FormControl({ value: 'Nancy', disabled: true }, Validators.required),//netPrice价格
      preReason: new FormControl({ value: 'Nancy', disabled: this.disa }, Validators.required),//prebook原因
      preDescription: new FormControl({ value: 'Nancy', disabled: this.disa }, Validators.required), //详细说明
      oitMonthEstimate: new FormControl({ value: 'Nancy', disabled: this.disa }, Validators.required), //oit预计月份
      shippingType: new FormControl({ value: 'Nancy', disabled: this.disa }, Validators.required),//运输方式
      arrivalDate: new FormControl({ value: 'Nancy', disabled: this.disa }, Validators.required),//客户要求时间
      ifcMonth: new FormControl({ value: 'Nancy', disabled: this.disa }, Validators.required), //预计ICF月份
      isPenalty: new FormControl({ value: 'Nancy', disabled: this.disa }, Validators.required), //延迟缴罚款
      downpaymentDate: new FormControl({ value: 'Nancy', disabled: this.disa }, Validators.required), //头期支付日期
      balancePaymentDate: new FormControl({ value: 'Nancy', disabled: this.disa }, Validators.required), //尾款支持日期
    });
    const roles = JSON.parse(localStorage.getItem("roles"));
    this.dataBase.tableColOffs = roles.some(val => (val == 'OA' || val == 'OA Leader' || val == 'CFC Leader' || val == 'ZPM' || val == 'PM Leader' || val == 'PM Leader_change' || val == 'Distributor leader' || val == 'ZSL' || val == 'COP Operation' || val == 'Finance: C&C Leader' || val == 'Cluster BP' || val == 'OM' || val == 'Sales Leader' || val == 'C&C Leader'))
  }
  ...
  GetDealLists(param) {
    return new Promise((resolve, reject) => {
      this.http.get(`/act/prebook/queryCpPreBook?dealFormId=` + param).subscribe(e => {
        resolve(e.data[0]);
  ...
  GetDealList() {
    this.deal_load = true;
    this.box = true;
    this.http.get(`/act/prebook/queryCpPreBook?dealFormId=` + this.dealFormIdinput).subscribe(e => {
  ...
  ifBusinessModel() {
    ...
    if (this.dataBase && this.dataBase.businessModel === 'DIRECT') {
      ...
      this.validateForm.get('balancePaymentDate')!.setValidators(Validators.required); //尾款日期
      this.validateForm.get('downpaymentDate')!.setValidators(Validators.required); //头款日期
