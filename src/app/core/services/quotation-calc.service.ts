...
export class QuotationCalcService implements OnInit {
  ...
  getResFromAmountSum(params: any) {
    ...
    calcParams['distributorprice'] =
      this.formatDistributorprice(ladderpriceRows);
    calcParams['promotions'] = this.formatPromotions(promotionRows);
    calcParams['specialamount'] = this.formatSpecialamount(
      currencyType,
      specialRows
    );
    ...
    calcParams['installfee'] = this.formatInstallfee(
      currencyType,
      installationfeeRows
    );
    ...
    if (params['unitTotalPrice'] && params['rawUnitTotalPrice']) {
      calcParams['currentUnitTotalPrice'] = params['unitTotalPrice']
        .toString()
        .replace(/[^0-9.-]/g, '');
    }
  ...
  formatTrainingfee({
    ...
    trainingcostList,
    otherTrainingList,
    newTrainingcostOnsite,
    newTrainingcostGroup,
    newTrainingcostOtherSum,
  }) {
    let result = 0;
    //开始计算一般培训费
    let sKey = currencyType == 'usd' ? 'usd' : 'rmb';
    if (!trainingcostVer || '' === trainingcostVer) {
      //老版培训费
    ...
    } else if ('2' === trainingcostVer) {
      //新版培训费
      result += newTrainingcostOnsite['total']
        ? Number(newTrainingcostOnsite['total']) || 0
        : 0; //现场培训总金额
      result += newTrainingcostGroup['total']
        ? Number(newTrainingcostGroup['total']) || 0
        : 0; //课程培训总金额
      result += newTrainingcostOtherSum['total']
        ? Number(newTrainingcostOtherSum['total']) || 0
        : 0; //其他培训总金额
    }
  ...
  formatGuaranteefee({ maintenanceVer, maintenanceList, maintenanceSum }) {
    let result = 0;
    if (!maintenanceVer || '' === maintenanceVer) {
      //老版保修费
