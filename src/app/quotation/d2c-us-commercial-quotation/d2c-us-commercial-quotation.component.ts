...
@Component({
  selector: 'd2c-us-commercial-quotation',
  templateUrl: './d2c-us-commercial-quotation.component.html',
  styleUrls: ['./d2c-us-commercial-quotation.component.scss']
})
export class D2cUsCommercialQuotationComponent extends FieldType implements OnInit {
  ...
  @ViewChild('crossRegionEmailFilesInput') crossRegionEmailFilesInput: ElementRef;
  ...
  //distributor new params start
  isDistributor: boolean = false;
  isBusinessFinished: boolean = false;
  priceNameInTable: string = '参考价格合计';
  funnel:any;

  //distributor new params ends

   isCrossRegion: boolean = false;
  // 判断进单类型
  dmsOrderTypeTag: 'normal' | 'stock' = 'normal';
  ...
  constructor(private commercialOrderService: CommercialOrderService,
    ...
    const pid = localStorage.getItem('processInstanceId');
    console.log('d2c template, pid', pid);
    const uri = '/act/dms/api/funnel/' + pid;
    this.funnel = null;
    this.http.get(uri).subscribe(res => {
      if('0000' === res.code) {
        console.log('funnel', res);
        if (res['data'] && 'CROSS_REGION' == res['data']['distributorRegion']) {
          this.isCrossRegion = true;
        }
        this.funnel = res.data['funnelDetail'];
        console.log('funnel in us', this.funnel);
      }else {
  ...
  ngOnInit() {
    ...
    //d2c new feature
    // this.isBusinessFinished = this.businessService.isBusinessFinished;
    // if (this.isBusinessFinished) {
    //   this.priceNameInTable = '设备总格（合计）';
    // }
    ...
    /**
     * 测试赋值
     * 这里可能要清空philipsAuthList等值 具体不是很清楚
     */
    // this.quotationForm.quotationBaseInfo.dmsOrderType = '普通订单';
    // if (this.quotationForm.quotationBaseInfo.dmsOrderType === '普通订单' || this.quotationForm.quotationBaseInfo.dmsOrderType === '') {
    //    this.isdmsOrderType = false;
    //  } else {
    //    this.isdmsOrderType = true;
    //  }
    if (this.quotationForm.quotationBaseInfo.dmsOrderType && '库存订单' == this.quotationForm.quotationBaseInfo.dmsOrderType) {
      this.dmsOrderTypeTag = 'stock';
    } else {
      this.dmsOrderTypeTag = 'normal';
    }
  ...
  initHiddenUIFields() {
    ...
    if (this.form.controls['salessapcode'] && formState['salessapcode']['hidden'] != '1') { //P2 added OA 输入销售SapCode
      ...
      // console.log('salesCode new', this.quotationForm.quotationBaseInfo.salesSapCode);
      if(this.quotationForm.quotationBaseInfo.salesSapCode) {
        this.salesCode1 = this.quotationForm.quotationBaseInfo.salesSapCode; //sapCode 实际上不是code1，此处仅用于回显
      }
      if (this.salesCode1 && !this.customDisabled['salesSapCode'] && this.salesCode1.indexOf('DISTRIBUTOR_USER') > -1) { //第一次进入设置salesSapCode时
        let pId = localStorage.getItem('processInstanceId');
        if (!pId) {
          console.log('localStorage获取pId失败...');
          return;
        }
        this.http.get('/act/task/queryLastestSalesCode/' + pId).subscribe(res => {
          console.log('latest sales code res => ', res);
          if('0000' == res.code && '' != res.data) {
            this.salesCode1 = res.data;
            setTimeout(() => {
  ...
  distributorAsBuyer(force: boolean = false) {
    if (force) {
      this.quotationForm.quotationBaseInfo.buyerName = this.quotationForm.quotationBaseInfo.distributorName;
    } else {
      if (this.currencyType != '1') {
        this.quotationForm.quotationBaseInfo.buyerName = this.quotationForm.quotationBaseInfo.distributorName;
      }
    }
    this.quotationForm.quotationBaseInfo.buyerTaxId = this.quotationForm.quotationBaseInfo.distributorTaxId;
    this.quotationForm.quotationBaseInfo.buyerAddress = this.quotationForm.quotationBaseInfo.distributorAddress;
    this.quotationForm.quotationBaseInfo.buyerContact = this.quotationForm.quotationBaseInfo.distributorContact;
    this.quotationForm.quotationBaseInfo.buyerContactPhone = this.quotationForm.quotationBaseInfo.distributorContactPhone;
    this.quotationForm.quotationBaseInfo.buyerEmail = this.quotationForm.quotationBaseInfo.distributorEmail;
    this.quotationForm.quotationBaseInfo.buyerFax = this.quotationForm.quotationBaseInfo.distributorFax;
  }
  ...
  uploadFile(flag) {
    ...
    }  else if (flag == 'crossRegionEmailFiles') {
      file = this.crossRegionEmailFilesInput.nativeElement.files[0];
      elementRef = this.crossRegionEmailFilesInput;
    }
  ...
  compareWithSales(a: any, b: any): boolean {
    return (a && b && a.code === b) || (a && b && a.sapCode === b);
  }
  ...
  /** dms stock order start */
  stockChangeEnduserType(event) {
    console.log('evnet => ', event);
    const eventStr = event || '';
    if (eventStr.indexOf('公立') > -1) {
      this.quotationForm.quotationBaseInfo.isPrivate = 0;
    } else if (eventStr.indexOf('民营') > -1) {
      this.quotationForm.quotationBaseInfo.isPrivate = 1;
    } else { //默认 公立
      this.quotationForm.quotationBaseInfo.isPrivate = 0;
    }
  }
